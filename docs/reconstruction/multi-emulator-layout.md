...
└── systems/
    ├── flycast/
    │   └── dc/                 # BIOS usado pelo Flycast/RetroArch
...
O scan somente verifica:

```text
<origem>/<machine>/<disk>.chd
```

Se não existir, o estado é `missing` imediatamente. Não há busca global por `.chd`, procura em outras machines, procura dentro de ZIP ou cálculo de SHA-1 durante o scan.
...
O scanner não faz `rglob('*')`. ROMs de ZIP somente são lidas quando nome, tamanho e CRC já coincidem e o LISTXML possui SHA-1 para confirmação. ROMs soltas seguem a mesma regra de nome → tamanho/CRC → SHA-1.
...
Isso evita o gargalo observado no teste de 6479 segundos, no qual o scan terminava e somente depois começava a construção do JSONL.

## Progresso e logs

O callback de progresso informa:

- machine concluída / total;
- ROMs verificadas;
- ROMs válidas;
- ROMs ausentes;
- CHDs presentes / total;
- GiB efetivamente lidos.

O logger registra início, cada machine concluída, resumo final e exceções. A reconstrução usa o mesmo princípio: callbacks de progresso/log são emitidos pelo worker para que a GUI nunca pareça ociosa.

## Limpeza de legado

O caminho antigo `app/mame/rom_scanner.py` foi removido, assim como os indexadores globais e scanners duplicados. A busca de ROMs reaproveitáveis deve permanecer fora do caminho crítico e ser usada pela reconstrução, não pelo inventário físico.

## Teste recomendado

Use exatamente o LISTXML e as mesmas origens do teste que levou 6479 segundos e compare:

1. tempo até o primeiro resultado;
2. tempo total;
3. crescimento do `.jsonl.tmp` durante o scan;
4. I/O do HDD;
5. CHDs ausentes resolvidos somente por `is_file()`;
