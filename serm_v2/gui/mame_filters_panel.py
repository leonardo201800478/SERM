...
class MameFilterState:
    ...
    fundamental: dict[str, bool] = field(
...
class MameAdvancedFiltersDialog(QDialog):
    ...
    def _general_tab(self) -> QWidget:
        ...
        self.title.setPlaceholderText("Romset name ou título do jogo")
        self.rom = QLineEdit()
        self.rom.setPlaceholderText("Nome da ROM / arquivo")
        self.parent_query = QLineEdit()
        self.parent_query.setPlaceholderText("Nome do parent")
        self.clone_query = QLineEdit()
        self.clone_query.setPlaceholderText("Nome do clone")
        self.full_text = QLineEdit()
        self.full_text.setPlaceholderText("Pesquisa no metadata do MAME")
        for label, widget in (
            ("Título / ROM:", self.title),
            ("ROM:", self.rom),
            ("Parent:", self.parent_query),
            ("Clone:", self.clone_query),
            ("Texto completo:", self.full_text),
        ):
        ...
        years.addWidget(QLabel("De"))
        years.addWidget(self.year_from)
        years.addWidget(QLabel("até"))
        years.addWidget(self.year_to)
        years.addStretch()
        form.addRow("Ano:", years)
        self.type_all = QRadioButton("Todos")
        self.type_parent = QRadioButton("Parent")
        self.type_clone = QRadioButton("Clone")
        self.type_all.setChecked(True)
        types = QHBoxLayout()
        types.addWidget(self.type_all)
        types.addWidget(self.type_parent)
        types.addWidget(self.type_clone)
        types.addStretch()
        form.addRow("Tipo:", types)
        self.emulation = QComboBox()
        self.emulation.addItem("Todos", "")
        [
            self.emulation.addItem(label, value)
            for value, label in self._LABELS["playability"].items()
        ]
        form.addRow("Emulação:", self.emulation)
        self.manufacturer = QListWidget()
        self.manufacturer.setSelectionMode(QListWidget.SelectionMode.MultiSelection)
        self.manufacturer.setMinimumHeight(130)
    ...
    def _driver_tab(self) -> QWidget:
        ...
        for key, title, hint in (
            ("content", "Tipo de conteúdo", "Arcade, console, handheld, mecânico, gambling etc."),
            ("hardware", "Hardware / plataforma", "Famílias de hardware descobertas pelo SERM."),
        ):
            box = QGroupBox(title)
    ...
    def _video_tab(self) -> QWidget:
        ...
        self.video_query = QLineEdit()
        self.video_query.setPlaceholderText("driver, vídeo, resolução, chipset…")
        self.screen_query = QLineEdit()
        self.screen_query.setPlaceholderText("tipo/tamanho de tela")
        self.orientation = QComboBox()
        self.orientation.addItems(["Todos", "Horizontal", "Vertical"])
        self.cabinet = QLineEdit()
        self.cabinet.setPlaceholderText("cabinet / gabinete")
        form.addRow("Vídeo:", self.video_query)
        form.addRow("Tela:", self.screen_query)
        form.addRow("Orientação:", self.orientation)
        form.addRow("Cabinet:", self.cabinet)
        note = QLabel(
            "Os campos textuais consultam o metadata efetivamente presente no catálogo do SERM."
        )
    ...
    def _audio_tab(self) -> QWidget:
        ...
        form = QFormLayout(page)
        form.setContentsMargins(14, 14, 14, 14)
        self.audio_query = QLineEdit()
        self.audio_query.setPlaceholderText("chip / driver de áudio")
        self.channels = QLineEdit()
        self.channels.setPlaceholderText("canais")
        form.addRow("Audio chip:", self.audio_query)
        form.addRow("Canais:", self.channels)
        return page
    ...
    def _categories_tab(self) -> QWidget:
        ...
        layout.addWidget(
            QLabel(
                "CATLIST do SERM — selecione categorias e subcategorias para INCLUIR somente essas machines."
            )
        )
        self._category_tree = QTreeWidget()
        self._category_tree.setHeaderLabels(["Categoria / subcategoria", "Machines"])
        self._category_tree.setColumnWidth(0, 460)
    ...
    def _extra_tab(self) -> QWidget:
        ...
        layout = QVBoxLayout(page)
        layout.setContentsMargins(10, 10, 10, 10)
        box = QGroupBox("Classificações adicionais")
    ...
    def _other_tab(self) -> QWidget:
        ...
        layout = QVBoxLayout(page)
        layout.setContentsMargins(10, 10, 10, 10)
        structure = QGroupBox("SET / estrutura")
        ...
        self.optional = QCheckBox("Incluir opcionais")
        self.working_other = QCheckBox("Working only")
        self.chd.setChecked(True)
        for i, w in enumerate(
            (self.bios, self.devices, self.chd, self.optional, self.working_other)
        ):
            grid.addWidget(w, i // 2, i % 2)
        layout.addWidget(components)
        exclusion = QGroupBox("Exclusões fundamentais")
        eg = QGridLayout(exclusion)
        self.fundamental = {}
        for i, (key, label) in enumerate(
            (
                ...
                ("console", "Consoles"),
                ("handheld", "Handhelds"),
                ("fruit_machines", "Fruit / gambling / redemption"),
                ("quiz", "Quiz / trivia"),
                ("tabletop", "Tabletop"),
            )
        ):
            w = QCheckBox(label)
            self.fundamental[key] = w
            eg.addWidget(w, i // 2, i % 2)
        layout.addWidget(exclusion)
        layout.addStretch()
        return page
    ...
    def set_facets(self, facets: dict[str, list[dict[str, object]]]) -> None:
        for key, widget in self._lists.items():
            ...
            for entry in facets.get(key, []):
                ...
                count = self._coerce_count(entry.get("count"))
    ...
    def set_categories(self, rows: list[dict[str, object]]) -> None:
        ...
        for row in rows:
            ...
            count = self._coerce_count(row.get("machines"))
            ...
            if parent is None:
                ...
                self._category_tree.addTopLevelItem(parent)
            if subcategory:
                child = QTreeWidgetItem([subcategory, str(count)])
                child.setData(0, Qt.ItemDataRole.UserRole, category)
                child.setData(0, Qt.ItemDataRole.UserRole + 1, subcategory)
    ...
    def values(self) -> dict[str, object]:
        type_filter = (
            "parent"
            if self.type_parent.isChecked()
            else "clone"
            if self.type_clone.isChecked()
            else "all"
        )
        categories: list[str] = []
        ...
        if self._category_tree is not None:
            for i in range(self._category_tree.topLevelItemCount()):
                ...
                if parent.checkState(0) == Qt.CheckState.Checked:
                    categories.append(parent.text(0))
                for j in range(parent.childCount()):
                    child = parent.child(j)
                    if child is None:
                        continue
                    if child.checkState(0) == Qt.CheckState.Checked:
                        subcategories.append(child.text(0))
        return {
            "title_query": self.title.text().strip(),
            "rom_query": self.rom.text().strip(),
            "parent_query": self.parent_query.text().strip(),
            "clone_query": self.clone_query.text().strip(),
            "full_text": self.full_text.text().strip(),
            "year_from": self.year_from.value() or None,
            "year_to": self.year_to.value() or None,
            "type_filter": type_filter,
            "video_query": self.video_query.text().strip(),
            "audio_query": self.audio_query.text().strip(),
            "screen_query": self.screen_query.text().strip(),
            "orientation": self.orientation.currentText().casefold(),
            "cabinet_query": self.cabinet.text().strip(),
            "channels_query": self.channels.text().strip(),
            "content": self._selected(self._lists["content"]),
            "hardware": self._selected(self._lists["hardware"]),
            "input": self._selected(self._lists["input"]),
            "wheel": self._selected(self._lists["wheel"]),
            "genre": self._selected(self._lists["genre"]),
            "series": self._selected(self._lists["series"]),
            "manufacturer": self._selected(self._lists["manufacturer"]),
            "playability": [self.emulation.currentData()] if self.emulation.currentData() else [],
            "categories": categories,
            "subcategories": subcategories,
            "mame_set_type": self.set_type.currentData(),
            "mame_clone_policy": self.clone_policy.currentData(),
            "mame_include_bios": self.bios.isChecked(),
            "mame_include_devices": self.devices.isChecked(),
            "mame_include_chd": self.chd.isChecked(),
            "mame_include_optional": self.optional.isChecked(),
            "mame_working_only": self.working.isChecked() or self.working_other.isChecked(),
            "fundamental": {key: widget.isChecked() for key, widget in self.fundamental.items()},
        }

    def apply_state(self, state: MameFilterState) -> None:
        self.title.setText(state.title_query)
        self.rom.setText(state.rom_query)
        self.parent_query.setText(state.parent_query)
        self.clone_query.setText(state.clone_query)
        self.full_text.setText(state.full_text)
        self.year_from.setValue(state.year_from or 0)
        self.year_to.setValue(state.year_to or 0)
        {"all": self.type_all, "parent": self.type_parent, "clone": self.type_clone}.get(
            state.type_filter, self.type_all
        ).setChecked(True)
        self.video_query.setText(state.video_query)
        self.audio_query.setText(state.audio_query)
        self.screen_query.setText(state.screen_query)
        self.cabinet.setText(state.cabinet_query)
        self.channels.setText(state.channels_query)
        idx = (
            self.orientation.findText(state.orientation.title())
        ...
        for key, widget in self._lists.items():
            ...
            for i in range(widget.count()):
                widget.item(i).setSelected(
                    str(widget.item(i).data(Qt.ItemDataRole.UserRole)) in selected
                )
        self.emulation.setCurrentIndex(
            max(0, self.emulation.findData(state.playability[0] if state.playability else ""))
        )
        self.set_type.setCurrentIndex(max(0, self.set_type.findData(state.mame_set_type)))
        self.clone_policy.setCurrentIndex(
            max(0, self.clone_policy.findData(state.mame_clone_policy))
        )
        ...
        self.working_other.setChecked(state.mame_working_only)
        for key, widget in self.fundamental.items():
            widget.setChecked(bool(state.fundamental.get(key, False)))
        if self._category_tree is not None:
            ...
            for i in range(self._category_tree.topLevelItemCount()):
                ...
                parent.setCheckState(
                    0, Qt.CheckState.Checked if parent.text(0) in cats else Qt.CheckState.Unchecked
                )
                for j in range(parent.childCount()):
                    child = parent.child(j)
                    if child is None:
                    ...
                    child.setCheckState(
                        0,
                        Qt.CheckState.Checked if child.text(0) in subs else Qt.CheckState.Unchecked,
                    )
...
class MameFiltersPanel(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        root.setSpacing(6)
        title = QLabel("MAME MACHINES  >  ARCADE")
        title.setProperty("role", "title")
        ...
        self.title_query = QLineEdit()
        self.title_query.setPlaceholderText("Romset name ou título do jogo")
        self.year_quick = QComboBox()
        self.year_quick.addItem("Todos os anos", None)
        self.genre_quick = QComboBox()
        self.genre_quick.addItem("Todos os gêneros", "")
        self.mamecab = QCheckBox("MameCab only")
        ...
        self.search_button.setToolTip("Executar o filtro com os critérios atuais")
        ...
        self.genre_quick.currentIndexChanged.connect(self._quick_changed)
        self.year_quick.currentIndexChanged.connect(self._quick_changed)
        self.show_clones.stateChanged.connect(self._quick_changed)
        self.mamecab.stateChanged.connect(self._quick_changed)
        grid.addWidget(QLabel("Nome:"), 0, 0)
        grid.addWidget(self.title_query, 0, 1, 1, 3)
        grid.addWidget(QLabel("Ano:"), 0, 4)
        grid.addWidget(self.year_quick, 0, 5)
        grid.addWidget(QLabel("Gênero:"), 0, 6)
        ...
        grid.addWidget(QLabel("Texto completo:"), 2, 0)
        grid.addWidget(self.full_text, 2, 1, 1, 7)
        root.addWidget(search)
        self.active = QLabel("Filtros adicionais: nenhum")
    ...
    def _set_summary(self, total: int, kept: int, excluded: int) -> None:
        ...
        if layout is None:
            layout = QHBoxLayout(self.summary)
            layout.setContentsMargins(0, 0, 0, 0)
            layout.setSpacing(6)
        while layout.count():
    ...
    def _scan_changed(self) -> None:
        ...
        if self._scan_path is None or not self._scan_path.is_file():
            ...
            self.search_button.setEnabled(False)
            return
        self.result.setText("Carregando catálogo do SERM e descobrindo filtros…")
    ...
    def _start_operation(self, operation: str) -> None:
        ...
        self._future = self._executor.submit(
            self._execute, operation, self._scan_path, self._state()
        )
    ...
    def _worker_ready(self, operation: str, payload) -> None:
        if operation == "facets":
            ...
            self._populate_quick_filters(payload)
            self._ensure_dialog()
            dialog = self._dialog
            if dialog is not None:
                dialog.set_facets(payload)
                dialog.set_categories(MameCategoryFilterService.tree())
    ...
    def _populate_quick_filters(self, facets: dict[str, list[dict[str, object]]]) -> None:
        self.genre_quick.blockSignals(True)
        self.genre_quick.clear()
        self.genre_quick.addItem("Todos os gêneros", "")
        [
            self.genre_quick.addItem(
                str(entry.get("value", "")).replace("_", " ").title(), str(entry.get("value", ""))
            )
            for entry in facets.get("genre", [])
        ]
        self.genre_quick.blockSignals(False)
        self.year_quick.blockSignals(True)
        self.year_quick.clear()
        self.year_quick.addItem("Todos os anos", None)
        years = sorted(
            {
                str(entry.get("value"))
                for entry in facets.get("year", [])
                if str(entry.get("value", "")).isdigit()
            },
            reverse=True,
        )
        [self.year_quick.addItem(year, int(year)) for year in years]
        self.year_quick.blockSignals(False)

    def _ensure_dialog(self) -> None:
        if self._dialog is None:
            self._dialog = MameAdvancedFiltersDialog(self)
    ...
    def _open_advanced(self) -> None:
        ...
        if dialog is None:
            return
        dialog.set_facets(self._facets)
        dialog.set_categories(MameCategoryFilterService.tree())
        dialog.apply_state(self._state())
        if dialog.exec() == QDialog.DialogCode.Accepted:
            self._apply_advanced_values(dialog.values())
            self._changed()

    def _apply_advanced_values(self, values: dict[str, object]) -> None:
        current = asdict(self._state())
        current.update(values)
        self._apply_state(MameFilterState(**current))

    def _state(self) -> MameFilterState:
        state = MameFilterState()
        state.title_query = self.title_query.text().strip()
        state.full_text = self.full_text.text().strip()
        state.mamecab_only = self.mamecab.isChecked()
        if self._dialog is not None:
            values = self._dialog.values()
            for key, value in values.items():
                if key in MameFilterState.__dataclass_fields__:
                    setattr(state, key, value)
        if self.genre_quick.currentData():
            state.genre = [str(self.genre_quick.currentData())]
        if self.year_quick.currentData() is not None:
            state.year_from = int(self.year_quick.currentData())
            state.year_to = int(self.year_quick.currentData())
        if not self.show_clones.isChecked():
            state.type_filter = "parent"
        state.mamecab_only = self.mamecab.isChecked()
        return state
    ...
    def _apply_state(self, state: MameFilterState) -> None:
        ...
        try:
            self.title_query.setText(state.title_query)
            self.full_text.setText(state.full_text)
            self.show_clones.setChecked(state.type_filter != "parent")
            self.mamecab.setChecked(state.mamecab_only)
            self.genre_quick.blockSignals(True)
            self.year_quick.blockSignals(True)
            try:
                genre_index = self.genre_quick.findData(state.genre[0] if state.genre else "")
                self.genre_quick.setCurrentIndex(max(0, genre_index))
                year_index = (
                    self.year_quick.findData(state.year_from)
                ...
                self.year_quick.blockSignals(False)
            if self._dialog is not None:
                self._dialog.apply_state(state)
    ...
    def _restore_last_state(self) -> None:
        try:
            ...
            if (
                saved_scan
                and self._scan_path
                and Path(saved_scan).resolve() != self._scan_path.resolve()
            ):
                return
            if isinstance(raw_state, dict):
                allowed = set(MameFilterState.__dataclass_fields__)
                state = MameFilterState(**{k: v for k, v in raw_state.items() if k in allowed})
                self._apply_state(state)
                self.result.setText(
                    "Último filtro restaurado automaticamente. Clique em BUSCAR para executar."
                )
    ...
    def _persist_last_state(self) -> None:
        ...
        self._last_state_path.write_text(
    ...
    def _persist_recent_profile(self, payload: dict) -> None:
        try:
            ...
            profiles = []
        signature = json.dumps(payload.get("state", {}), sort_keys=True, ensure_ascii=False)
        profiles = [
            p
            for p in profiles
            if isinstance(p, dict)
            and json.dumps(p.get("state", {}), sort_keys=True, ensure_ascii=False) != signature
        ]
        profiles.insert(0, payload)
        self._recent_profiles_path.parent.mkdir(parents=True, exist_ok=True)
        self._recent_profiles_path.write_text(
            json.dumps(profiles[: self._RECENT_LIMIT], indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
    ...
    def _changed(self, *_args) -> None:
        ...
        self._update_active_summary(self._state())

    def _search_clicked(self) -> None:
        if not self._facets_loaded or self._scan_path is None:
            return
        self._persist_last_state()
        self.result.setText("Executando filtro no catálogo relacional do SERM…")
        self.search_button.setEnabled(False)
        self._start_operation("preview")

    def _update_active_summary(self, state: MameFilterState) -> None:
        parts: list[str] = []
        if state.title_query:
            parts.append(f'Título="{state.title_query}"')
        if state.full_text:
            parts.append(f'Texto="{state.full_text}"')
        if state.year_from:
            parts.append(
                f"Ano={state.year_from}"
                if state.year_from == state.year_to
                else f"Ano={state.year_from}-{state.year_to}"
            )
        if state.genre:
            parts.append("Gênero=" + ", ".join(state.genre))
        if state.content:
            parts.append("Conteúdo=" + ", ".join(state.content))
        if state.manufacturer:
            parts.append("Fabricante=" + ", ".join(state.manufacturer[:3]))
        if state.categories or state.subcategories:
            parts.append(
                f"CATLIST={len(state.categories) + len(state.subcategories)} selecionada(s)"
            )
        if state.type_filter != "all":
            parts.append("Parents only" if state.type_filter == "parent" else "Clones only")
        if state.mame_set_type != "split":
            parts.append(state.mame_set_type)
        if state.mame_working_only:
            parts.append("Working")
        if not state.mamecab_only:
            parts.append("MameCab OFF")
        self.active.setText("Filtros ativos: " + (" • ".join(parts) if parts else "nenhum"))

    def _clear_filters(self) -> None:
        self._apply_state(MameFilterState())
        self.genre_quick.setCurrentIndex(0)
    ...
    def _format_preview(payload: dict) -> str:
        ...
        reason_text = (
            ", ".join(
                f"{str(k).replace('_', ' ')}: {int(v):,}" for k, v in list(reasons.items())[:6]
            )
            or "nenhuma"
        )
    ...
    def _save_profile(self) -> None:
        ...
        profiles = [
            p
            for p in profiles
            if not isinstance(p, dict) or p.get("profile_id") != state["profile_id"]
        ]
        profiles.append(state)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(profiles, indent=2, ensure_ascii=False), encoding="utf-8")
        self.result.setText("Perfil salvo e última configuração registrada automaticamente.")

    def _apply(self) -> None:
        if (
            self._scan_path is None
            or not self._facets_loaded
            or (self._future is not None and not self._future.done())
        ):
            return
        self._persist_last_state()
    ...
    def _worker_failed(self, message: str) -> None:
        ...
        self.search_button.setEnabled(self._facets_loaded)
