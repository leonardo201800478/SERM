...
class MameCatalogNormalizer:
    ...
    def normalize(
        self, db: sqlite3.Connection, import_id: int, root: ET.Element
    ) -> dict[str, int | float]:
        ...
        for start in range(0, len(machines), self.BATCH_SIZE):
            ...
            for machine in batch:
                ...
                if not name:
                    raise MameCatalogNormalizationError(
                        f"Machine sem nome no import_id={import_id}."
                    )
    ...
    def _insert_machine(db: sqlite3.Connection, import_id: int, machine: ET.Element) -> int:
        ...
        db.execute(
            ...
            (
                machine_id,
                d.get("status") or d.get("emulation"),
                d.get("status"),
                d.get("savestate"),
                d.get("requires_artwork"),
                d.get("unofficial"),
                d.get("nosoundhardware"),
                d.get("incomplete"),
            ),
        )
        return machine_id
    ...
    def _insert_children(
        ...
        for element in machine.findall("biosset"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_biosset(machine_id,name,description,default_flag) VALUES(?,?,?,?)",
        ...
        for element in machine.findall("rom"):
            a = element.attrib
            db.execute(
                """INSERT INTO mame_rom(machine_id,name,bios,size,crc,sha1,md5,merge,region,offset,status,optional,dispose)
                         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    machine_id,
                    a.get("name"),
                    a.get("bios"),
                    self._int(a.get("size")),
                    a.get("crc"),
                    a.get("sha1"),
                    a.get("md5"),
                    a.get("merge"),
        ...
        for element in machine.findall("disk"):
            a = element.attrib
            db.execute(
                """INSERT INTO mame_disk(machine_id,name,md5,sha1,merge,region,index_value,writable,status,optional)
                         VALUES(?,?,?,?,?,?,?,?,?,?)""",
                (
                    machine_id,
                    a.get("name"),
        ...
        for element in machine.findall("device_ref"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_device_ref(machine_id,name,tag,mandatory) VALUES(?,?,?,?)",
        ...
        for element in machine.findall("sample"):
            db.execute(
                ...
                (machine_id, element.attrib.get("name")),
            )
            totals["samples"] += 1
        for element in machine.findall("chip"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_chip(machine_id,type,tag,name,clock) VALUES(?,?,?,?,?)",
    ...
    def _insert_input(
        ...
        a = element.attrib
        cur = db.execute(
            "INSERT INTO mame_input(machine_id,players,coins,service,tilt) VALUES(?,?,?,?,?)",
            (
                machine_id,
                self._int(a.get("players")),
                self._int(a.get("coins")),
                self._int(a.get("service")),
        ...
        for control in element.findall("control"):
            c = control.attrib
            db.execute(
                """INSERT INTO mame_control
                (input_id,type,player,buttons,minimum,maximum,sensitivity,keydelta,reverse,ways)
                VALUES(?,?,?,?,?,?,?,?,?,?)""",
                (
                    input_id,
                    c.get("type"),
                    self._int(c.get("player")),
                    self._int(c.get("buttons")),
                    self._int(c.get("minimum")),
                    self._int(c.get("maximum")),
                    self._int(c.get("sensitivity")),
                    self._int(c.get("keydelta")),
                    c.get("reverse"),
                    c.get("ways"),
                ),
            )
    ...
    def _insert_named_children(
        ...
        for element in machine.findall("port"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_port(machine_id,tag,type,mask,defvalue,condition) VALUES(?,?,?,?,?,?)",
                (
                    machine_id,
                    a.get("tag"),
                    a.get("type"),
        ...
        for element in machine.findall("adjuster"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_adjuster(machine_id,name,default_value,min_value,max_value) VALUES(?,?,?,?,?)",
                (machine_id, a.get("name"), a.get("default"), a.get("minimum"), a.get("maximum")),
            )
            totals["adjusters"] += 1
        driver = machine.find("driver")
        if driver is not None:
            a = driver.attrib
            db.execute(
                """INSERT INTO mame_driver(machine_id,status,emulation,color,sound,graphic,cocktail,protection,savestate,requires_artwork,unofficial,incomplete,notes)
                         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
                    machine_id,
                    a.get("status"),
        ...
        for element in machine.findall("feature"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_feature(machine_id,type,status) VALUES(?,?,?)",
                (machine_id, a.get("type"), a.get("status")),
            )
            totals["features"] += 1
        for element in machine.findall("device"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_device(machine_id,type,tag,name,clock) VALUES(?,?,?,?,?)",
                (machine_id, a.get("type"), a.get("tag"), a.get("name"), a.get("clock")),
            )
            totals["devices"] += 1
        for element in machine.findall("slot"):
            ...
            for option in element.findall("slotoption"):
                o = option.attrib
                db.execute(
                    "INSERT INTO mame_slot_option(slot_id,name,devname,is_default) VALUES(?,?,?,?)",
                    (sid, o.get("name", ""), o.get("devname"), o.get("default")),
                )
                totals["slot_options"] += 1
        for element in machine.findall("softwarelist"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_softwarelist(machine_id,tag,name,status,filter) VALUES(?,?,?,?,?)",
                (machine_id, a.get("tag"), a.get("name"), a.get("status"), a.get("filter")),
            )
            totals["softwarelists"] += 1
        for element in machine.findall("ramoption"):
            a = element.attrib
            db.execute(
                "INSERT INTO mame_ramoption(machine_id,name,default_value) VALUES(?,?,?)",
