...
class MameFilterV2Service:
        ...
        with sqlite3.connect(database_path(), timeout=60.0) as db:
            ...
            for row in rows:
                ...
                is_bios, is_device, is_mechanical = (
                    cls._mame_flag(isbios),
                    cls._mame_flag(isdevice),
                    cls._mame_flag(ismechanical),
                )
                category_values = [v.strip() for v in str(categories or "").split(",") if v.strip()]
                subcategory_values = [
                    v.strip() for v in str(subcategories or "").split(",") if v.strip()
                ]
                ...
                if (
                    ...
                    and not any(v.casefold() in non_arcade_categories for v in category_values)
                ):
                    category_values.append("arcade")
                folder_filters = {
                    key: sorted(values.get(name, set())) for key, values in folder_maps.items()
                }
                genre_values = folder_filters.get("genre", [])
                series_values = folder_filters.get("series", [])
                working = bool(folder_filters.get("working")) and not bool(
                    folder_filters.get("not_working")
                )
                ...
                elif cls._has_value(status, {"preliminary"}) or cls._has_value(
                    emu, {"preliminary"}
                ):
                ...
                metadata = {
                    ...
                    "disk_count": int(disk_count or 0),
                    "sample_count": int(sample_count or 0),
                    "bios_count": int(bios_count or 0),
                ...
                games.append(
                    ArcadeGame(
                        ...
                        display_name=str(description or name),
    ...
    @classmethod
    def _folder_maps(
        cls, db: sqlite3.Connection, machine_names: set[str]
        ...
        return {
            key: [
                ...
                for value, count in sorted(
                    counter.items(), key=lambda pair: (-pair[1], str(pair[0]).casefold())
                )
            ]
    ...
    def _is_arcade(game: ArcadeGame) -> bool:
        return (
            ArcadeFilterEngine()._classifier.classify(game).content_type is ArcadeContentType.ARCADE
        )
    ...
    def _candidate_names(cls, games: list[ArcadeGame], state) -> set[str] | None:
        ...
        queries = {
            key: str(getattr(state, key, "") or "").strip().casefold()
            for key in (
                "title_query",
                "full_text",
                "rom_query",
        ...
        def match(game: ArcadeGame) -> bool:
            ...
            if (
                queries["title_query"]
                and queries["title_query"]
                not in f"{game.machine_name} {game.display_name}".casefold()
            ):
                return False
            if queries["full_text"] and queries["full_text"] not in meta:
                return False
            if (
                queries["rom_query"]
                and queries["rom_query"] not in str(game.metadata.get("rom_names", "")).casefold()
            ):
                return False
            if (
                queries["parent_query"]
                and queries["parent_query"] not in str(game.parent_name or "").casefold()
            ):
                return False
            if (
                queries["clone_query"]
                and queries["clone_query"] not in game.machine_name.casefold()
            ):
                return False
            ...
            if year_from is not None or year_to is not None:
                if not cls._is_arcade(game) or year is None:
                    return False
                if year_from is not None and year < int(year_from):
                    return False
                if year_to is not None and year > int(year_to):
                    return False
            if orientation == "horizontal" and not cls._is_horizontal(game):
                return False
            if orientation == "vertical" and not cls._is_vertical(game):
                return False
            for key in (
                "video_query",
                "audio_query",
                ...
                if queries[key] and queries[key] not in meta:
                    return False
            return True
    ...
    def _rules(cls, state, games: list[ArcadeGame] | None = None) -> FilterRules:
        ...
        excluded_genres = (
            {ArcadeGenre.DANCE} if bool(state.fundamental.get("dance", False)) else set()
        )
        ...
        return FilterRules(
            included_machine_names=frozenset(candidate_names)
    ...
    @staticmethod
    def _trace_key(item) -> str:
        trace = getattr(item, "trace", None)
        if trace is None:
            return ""
        if getattr(trace, "rule", None):
            return trace.rule
        stage = getattr(trace, "stage", None)
        return (
            stage.value if stage is not None and getattr(stage, "value", None) is not None else ""
        )

    @classmethod
    def preview(cls, path: Path, state) -> dict:
        _, _, result = cls._result(path, state)
        reasons = Counter(cls._trace_key(item) for item in result.excluded)
        return {
            "unit": "machines",
            ...
            "stage_counts": {
                stage.value: count for stage, count in result.counts_after_stage.items()
            },
        }
    ...
    def apply(cls, path: Path, state) -> dict:
        ...
        kept_evidence = [
            ...
            if str(item.get("machine_name") or item.get("machine") or item.get("name") or "")
            in kept_names
        ]
        reasons = Counter(cls._trace_key(item) for item in result.excluded)
        run_id = uuid4().hex[:16]
        ...
        output_path = (
            out_dir / f"MAME_{label}_{payload.get('scan_type', 'arcade')}_FILTER_{run_id}.json"
        )
        output = {
