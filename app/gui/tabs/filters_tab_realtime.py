"""Aba de filtragem de ROMs e gerenciamento de categorias (versão responsiva)."""
...
class FiltersTab(QWidget):
    """Interface principal para configuração e aplicação dos filtros do MAME (assíncrona)."""

    # Sinais da classe (mantidos da versão normal)
    ...
    # Sinal interno para resultado do cálculo assíncrono
    filter_result_signal = Signal(int, object, object)

    # ----------------------------------------------------------------------
    # CategoryChip com sinal próprio (desacoplado)
    # ----------------------------------------------------------------------
    class CategoryChip(QPushButton):
        ...
        def __init__(self, category_name: str, display_name: str, count: int, parent=None):
        ...
        def toggle_state(self) -> None:
            ...
            self.update_style()
            self.state_changed.emit(self.category_name, self.state)

        def update_style(self) -> None:
            base_style = "border: 1px solid #888; border-radius: 3px; padding: 2px 6px; font-size: 8pt;"
        ...
        def set_state(self, state: int) -> None:
            self.state = self.STATE_EXCLUDE if state == self.STATE_EXCLUDE else self.STATE_NORMAL
            self.update_style()

    # ----------------------------------------------------------------------
    # MacroCategoryChip (copiado da versão normal)
    # ----------------------------------------------------------------------
    class MacroCategoryChip(QPushButton):
        ...
        def toggle_state(self) -> None:
            self.state = self.STATE_NORMAL if self.state == self.STATE_EXCLUDE else self.STATE_EXCLUDE
            self.update_style()
            self.state_changed.emit(self.macro_name, self.state)

        def update_style(self) -> None:
            base_style = "border: 1px solid #666; border-radius: 4px; padding: 4px 10px; font-weight: bold; font-size: 8pt;"
            if self.state == self.STATE_NORMAL:
                self.setStyleSheet(f"background-color: #d0d0d0; color: black; {base_style}")
            elif self.state == self.STATE_EXCLUDE:
                self.setStyleSheet(f"background-color: #cc0000; color: white; {base_style}")
            else:
                self.setStyleSheet(f"background-color: #ffb84d; color: black; {base_style}")

        def set_state(self, state: int) -> None:
            if state not in (self.STATE_NORMAL, self.STATE_EXCLUDE, self.STATE_MIXED):
    ...
    def __init__(self, parent=None, db: Database = None):
        ...
        # Controle do recálculo assíncrono
        ...
        self.filter_result_signal.connect(self._on_filter_calculation_finished)
    ...
    # ----------------------------------------------------------------------
    # Interface (copiada da normal, com o grupo de macro-categorias)
    # ----------------------------------------------------------------------
    def _setup_ui(self) -> None:
        ...
        btn_import_cat.setToolTip("Importa o catver.ini da pasta 'folders' do MAME, agrupando pelo primeiro nível.")
        ...
        status_options = [("working", "Working"), ("imperfect", "Imperfect"), ("not_working", "Not Working")]
        ...
        self.lbl_excluded_categories.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
        ...
        info_layout.addRow("Categorias excluídas:", self.lbl_excluded_categories)
        ...
        self.macro_grid.setAlignment(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
        ...
        legend = QLabel(
            "Clique na categoria para excluir (vermelho) ou restaurar ao estado normal (cinza)."
        )
        ...
        cat_container.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Maximum)
    ...
    def _load_categories(self) -> None:
        ...
        for idx, cat in enumerate(all_cats):
            ...
            chip.state_changed.connect(self._on_category_chip_changed)   # <-- signal desacoplado
    ...
    def _update_excluded_categories_info(self, excluded_categories: List[str]) -> None:
        if not excluded_categories:
            ...
            return
        display_names = []
        for cat_name in excluded_categories:
            chip = self.category_chips.get(cat_name)
            display_names.append(chip.text() if chip else cat_name)
        summary = f"{len(display_names)}"
        self.lbl_excluded_categories.setText(summary + ": " + ", ".join(display_names))
        self.lbl_excluded_categories.setToolTip("\n".join(display_names))

    def on_category_changed(self, category_name: str, state: int) -> None:
        """Ponto de entrada quando uma categoria é alterada (chamado pelo chip via signal)."""
        excluded = self._get_excluded_categories()
        self._update_excluded_categories_info(excluded)
        self._refresh_macro_chip_states()   # sincroniza macro-chips
        logger.debug("Categoria alterada: %s -> %s; excluídas=%s",
                     category_name, "EXCLUDE" if state == self.CategoryChip.STATE_EXCLUDE else "NORMAL", excluded)
        self._on_filters_changed()

    # ----------------------------------------------------------------------
    # Macro categorias (copiado da normal)
    # ----------------------------------------------------------------------
    ...
    def _load_macro_categories(self) -> None:
        ...
        for idx, group in enumerate(macro_groups):
            row = idx // cols
            col = idx % cols
            chip = self.MacroCategoryChip(group["macro_name"], group["count"], self)
            chip.state_changed.connect(self._on_macro_category_chip_changed)
    ...
    def _refresh_macro_chip_states(self) -> None:
        macro_groups = self.filter_service.get_macro_categories_with_counts()
        macro_categories = {g["macro_name"]: g["categories"] for g in macro_groups}

        for macro_name, chip in self.macro_chips.items():
            relevant = [self.category_chips[name] for name in macro_categories.get(macro_name, []) if name in self.category_chips]
            if not relevant:
                chip.set_state(self.MacroCategoryChip.STATE_NORMAL)
                continue
            excluded_count = sum(1 for c in relevant if c.state == self.CategoryChip.STATE_EXCLUDE)
            if excluded_count == 0:
                chip.set_state(self.MacroCategoryChip.STATE_NORMAL)
            elif excluded_count == len(relevant):
                chip.set_state(self.MacroCategoryChip.STATE_EXCLUDE)
            else:
                chip.set_state(self.MacroCategoryChip.STATE_MIXED)

    def _on_macro_category_chip_changed(self, macro_name: str, state: int) -> None:
        logger.debug("Signal MacroCategoryChip recebido: %s -> %s", macro_name,
                     "EXCLUDE" if state == self.MacroCategoryChip.STATE_EXCLUDE else "NORMAL")
        self.on_macro_category_changed(macro_name, state)

    def on_macro_category_changed(self, macro_name: str, state: int) -> None:
        macro_groups = self.filter_service.get_macro_categories_with_counts()
        category_names = next((g["categories"] for g in macro_groups if g["macro_name"] == macro_name), [])
        target_state = self.CategoryChip.STATE_EXCLUDE if state == self.MacroCategoryChip.STATE_EXCLUDE else self.CategoryChip.STATE_NORMAL

        for name in category_names:
            chip = self.category_chips.get(name)
            if chip:
                chip.set_state(target_state)   # não emite signal

        self._update_excluded_categories_info(self._get_excluded_categories())
        self._refresh_macro_chip_states()
        self._on_filters_changed()   # recalcula uma única vez
    ...
    # ========================================================================
    # Importação de categorias, CHD scan (iguais)
    # ========================================================================
    ...
    def _import_categories(self) -> None:
        ...
        if not default_ini.exists():
            ...
            if reply != QMessageBox.StandardButton.Yes:
                return
            file_path, _ = QFileDialog.getOpenFileName(self, "Selecionar catver.ini", "", "Arquivos INI (*.ini);;Todos os arquivos (*)")
            if not file_path:
                return
    ...
    def _scan_chd_sizes(self) -> None:
        ...
        if not self.config.ini_path or not self.config.ini_path.exists():
            QMessageBox.warning(
                self,
                "Erro",
                "Selecione o mame.ini na aba Diretórios primeiro (é de lá que vem o 'rompath' onde os .chd ficam)."
            )
            return
        try:
        ...
        def scan_task():
            try:
                ...
                self.finish_signal.emit(
                    True,
                    f"Scanner de CHD concluído.\n{len(chd_sizes)} arquivo(s) .chd encontrados nos rompaths.\n{updated} registro(s) atualizados no banco."
                )
            except Exception as e:
    ...
    def _load_criteria(self, criteria: FilterCriteria) -> None:
        ...
        try:
            ...
            for name, chip in self.category_chips.items():
                chip.set_state(self.CategoryChip.STATE_EXCLUDE if name in exclude_set else self.CategoryChip.STATE_NORMAL)
        finally:
    ...
    def _on_filters_changed(self) -> None:
        """Agenda recálculo assíncrono das estatísticas."""
    ...
    def _start_filter_calculation(self) -> None:
        ...
        def worker() -> None:
            ...
            try:
                ...
                self.filter_result_signal.emit(generation, result, None)
            except Exception as exc:
                logger.error("Erro no cálculo assíncrono dos filtros: %s", exc, exc_info=True)
                self.filter_result_signal.emit(generation, None, str(exc))
            finally:
                if conn is not None:
                    conn.close()

        threading.Thread(target=worker, name="filter-stats-worker", daemon=True).start()

    def _on_filter_calculation_finished(self, generation: int, result: dict | None, error: str | None) -> None:
        self._filter_calculation_running = False
        ...
        if error is not None or result is None:
            ...
            self._update_excluded_categories_info(self.current_criteria.exclude_categories)
            return
        ...
        if result["unscanned_chds"] > 0:
            size_str += f"  (⚠ {result['unscanned_chds']} CHD(s) sem tamanho lido — use 'Escanear tamanho dos CHDs')"
    ...
    def _apply_filters(self) -> None:
        """Agenda recálculo assíncrono (mantido para compatibilidade)."""
        criteria = self._get_criteria_from_ui()
        self.current_criteria = criteria
        self._filter_generation += 1
        self._pending_filter_generation = self._filter_generation
    ...
    def _on_progress_update(self, value: int, message: str) -> None:
        if self.main_window and hasattr(self.main_window, "status_bar"):
            self.main_window.status_bar.showMessage(message)

    def _on_category_chip_changed(self, category_name: str, state: int) -> None:
        logger.debug("Signal CategoryChip recebido: %s -> %s", category_name,
                     "EXCLUDE" if state == self.CategoryChip.STATE_EXCLUDE else "NORMAL")
        self.on_category_changed(category_name, state)
    ...
    def _update_database_info(self) -> None:
        try:
            ...
            logger.error(f"Erro ao atualizar informações do banco: {e}", exc_info=True)
    ...
    # ========================================================================
    # Perfis: CRUD (copiados da normal)
    # ========================================================================
    ...
    def _create_new_profile(self) -> None:
        ...
        profile = FilterProfile(name=name, description=desc_edit.text().strip(), criteria=self._get_criteria_from_ui())
    ...
    def _delete_profile(self) -> None:
        ...
        if not profile:
            return
        reply = QMessageBox.question(self, "Confirmar", f"Excluir o perfil '{profile.name}'?",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
