...
class SystemOptimizationService:
    ...
    def __init__(
        ...
        self.catalog_path = (
            self.project_root / "data" / "launchbox" / "system_optimizations.json"
        )
    ...
    def _key(value: str | None) -> str:
        ...
        return " ".join(
            (value or "").replace("_", " ").replace("-", " ").split()
        ).casefold()
    ...
    def _parse_profile(
        self,
        item: dict[str, Any],
    ) -> SystemOptimizationProfile | None:
        ...
        if isinstance(cores_raw, dict):
            for core_name, raw_core in cores_raw.items():
                ...
                if not isinstance(targets, dict):
                    targets = {}
                core_optimizations[core] = CoreOptimization(
                    core=core,
    ...
    def profiles_for_system(
        self,
        system_name: str,
        system_id: str = "",
    ) -> list[SystemOptimizationProfile]:
    ...
    def _write_managed(self, path: Path, content: str, overwrite: bool) -> None:
        ...
        if path.exists() and not self._is_managed(path) and not overwrite:
            raise FileExistsError(
                f"Arquivo existente não gerenciado pelo Arcade Manager: {path}"
            )
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(self._managed_content(content), encoding="utf-8")

    def _prepare_core_files(
        self,
        profile: SystemOptimizationProfile,
        optimization: CoreOptimization,
    ) -> dict[Path, str]:
        ...
        if profile.shader and "override" in files:
            ...
            lines = [
                ...
                if not line.strip().startswith("video_shader =")
            ]
        ...
        return {
            self._core_target(optimization, key): content
    ...
    def apply(
        ...
        *,
        ...
        if profile not in self.profiles_for_system(system_name, system_id):
            raise ValueError(
                f"O perfil '{profile.name}' não é compatível com '{system_name}'."
            )
        ...
        if profile.shader:
            shader_path = self._shader_path(profile.shader)
            self._write_managed(shader_path, profile.shader.content, overwrite)
            written.append(shader_path)

        for optimization in profile.core_optimizations.values():
            for path, content in self._prepare_core_files(profile, optimization).items():
    ...
    def remove(
        ...
        return {
            ...
            "backups": [],
            "warnings": [
                f"Arquivo externo preservado: {path}" for path in sorted(skipped)
            ],
        }
