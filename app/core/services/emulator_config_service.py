"""Gerenciamento seguro de arquivos de configuração de emuladores.

Regra central:

    arquivo válido     -> reutilizar
    arquivo ausente    -> gerar somente se houver comando de geração
    arquivo corrompido -> preservar backup e gerar somente se houver comando

O serviço nunca substitui silenciosamente uma configuração válida do usuário.
A execução de comandos é silenciosa no Windows e não utiliza ``shell=True``.
"""
...
class EmulatorConfigSpec:
    """Descreve um arquivo que pode ser reutilizado ou regenerado."""
...
@dataclass(frozen=True, slots=True)
class EnsureResult:
    """Resultado auditável da política de configuração."""
...
class EmulatorConfigService:
    """Aplica a política de não sobrescrever configurações válidas."""

    DEFAULT_TIMEOUT = 60

    def ensure(self, spec: EmulatorConfigSpec, *, timeout: int | None = None) -> EnsureResult:
        """Garante que o arquivo exista e seja estruturalmente válido.

        A ordem é deliberada: primeiro valida o arquivo existente. Somente
        quando ele não existe ou está inválido o gerador é executado.
        """
        ...
        stdout, stderr = self._run_generator(
            spec.generator_command,
            cwd=spec.cwd or path.parent,
            timeout=timeout or self.DEFAULT_TIMEOUT,
        )

        if not path.is_file() or not self._is_valid(spec):
            raise RuntimeError(
                f"O gerador de {spec.emulator}/{spec.name} terminou sem produzir uma configuração válida: {path}"
    ...
    def _is_valid(spec: EmulatorConfigSpec) -> bool:
        """Valida usando a função específica ou a validação pelo formato."""
        try:
    ...
    def _validate_ini_like(path: Path) -> bool:
        ...
        if not lines:
            return False
        # Configurações do Flycast/Supermodel/FBNeo podem usar tanto
        # key=value quanto key <espaços> value. Exigimos pelo menos uma linha
        # que pareça uma atribuição ou seção, sem impor um dialeto único.
    ...
    def _validate_dat(path: Path) -> bool:
        ...
        markers = ("game", "rom", "setname", "datfile", "finalburn")
        return any(marker in text for marker in markers)

    @staticmethod
    def _backup_invalid(path: Path) -> Path:
        """Preserva uma configuração inválida antes de regenerá-la."""
    ...
    def _run_generator(command: Sequence[str], *, cwd: Path, timeout: int) -> tuple[str, str]:
        """Executa o gerador sem shell e sem janela de console no Windows."""
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"Comando de geração terminou com código {result.returncode}: {' '.join(command)}\n"
...
def validate_mame_ini(path: Path) -> bool:
    """Valida MAME.INI usando o parser já existente no projeto."""
    try:
...
def validate_xml(path: Path) -> bool:
    """Valida XML completo sem carregar o conteúdo inteiro como string."""
    try:
        ET.parse(path)
...
def validate_flycast_cfg(path: Path) -> bool:
    """Valida emu.cfg do Flycast como texto não vazio e estruturado."""
    return EmulatorConfigService._validate_ini_like(path)


def validate_supermodel_ini(path: Path) -> bool:
    """Valida Supermodel.ini como configuração estruturada."""
    return EmulatorConfigService._validate_ini_like(path)


def validate_fbneo_ini(path: Path) -> bool:
    """Valida o arquivo INI principal do FBNeo."""
    return EmulatorConfigService._validate_ini_like(path)


def validate_fbneo_dat(path: Path) -> bool:
    """Valida o DAT textual gerado pelo FBNeo."""
