...
=======
ROMs são indexadas por CRC/tamanho. CHDs são tratados separadamente como
arquivos externos aos ZIPs e registrados explicitamente no manifesto.
>>>>>>> 5bc4f0bc4515edb1a835cd8055da8b3eccc6faac
"""
...
class PhysicalRomScanner:
    """Indexa fisicamente ROMs e CHDs sem modificar as fontes."""
...
=======
    ...
    def scan(self, machine_names: Iterable[str] | None = None, run_id: int | None = None,
             ...
             machine_disks: dict[str, list[dict]] | None = None) -> dict:
...
>>>>>>> 5bc4f0bc4515edb1a835cd8055da8b3eccc6faac
        ...
        stats = {
            ...
            "chds": 0, "chds_valid": 0, "chds_missing": 0, "chds_invalid": 0, "chds_errors": 0,
            "bytes_read": 0, "valid": 0, "sha1_mismatch": 0, "unmatched": 0, "read_errors": 0,
            "seconds": 0.0, "status": "running",
        }
        pending = 0
        logger.info("Scan físico iniciado: %d ROMs e %d CHDs esperados, %d origem(ns).", stats["expected_roms"], stats["expected_chds"], len(self.source_dirs))
...
=======
                    if not path.is_file() or path.suffix.lower() == ".chd":
                        continue
>>>>>>> 5bc4f0bc4515edb1a835cd8055da8b3eccc6faac
            ...
            for machine_name, disks in self._expected_disks.items():
                for disk in disks:
                    ...
                    if status == "valid": stats["chds_valid"] += 1
                    elif status == "missing": stats["chds_missing"] += 1
                    elif status == "invalid": stats["chds_invalid"] += 1
                    else: stats["chds_errors"] += 1
                    if progress:
            ...
            logger.info("Scan físico concluído: archives=%d members=%d loose=%d chds=%d/%d válidos bytes=%d valid=%d sha1_mismatch=%d unmatched=%d errors=%d tempo=%.2fs", stats["archives"], stats["members"], stats["loose"], stats["chds"], stats["chds_valid"], stats["bytes_read"], stats["valid"], stats["sha1_mismatch"], stats["unmatched"], stats["read_errors"], stats["seconds"])
            return stats
...
>>>>>>> 5bc4f0bc4515edb1a835cd8055da8b3eccc6faac
    ...
    def _build_expected_disks(self, machine_names: Iterable[str] | None) -> dict[str, list[dict]]:
        """Carrega os CHDs esperados diretamente da tabela ``disk`` do catálogo."""
        names = [str(n).strip() for n in (machine_names or []) if str(n).strip()]
...
>>>>>>> 5bc4f0bc4515edb1a835cd8055da8b3eccc6faac
    ...
    def _scan_expected_chd(self, machine_name: str, disk: dict, cancelled: Callable[[], bool] | None) -> dict:
        """Localiza e valida somente o CHD esperado."""
        ...
        for base in self.source_dirs:
            ...
            # Compatibilidade com organização plana.
        ...
        try:
            ...
            return {"status": "valid" if valid else "invalid", "source_path": str(found), "actual_size": actual_size, "actual_sha1": actual_sha1, "error": None if valid else "tamanho ou SHA-1 incompatível"}
        except Exception as exc:
    ...
    def _record_matches(self, scan_id: int, source_path: Path, archive_member: str | None, source_kind: str, size: int, crc: str, sha1: str, candidates: list[dict], result: dict) -> int:
        """Relaciona conteúdo físico com todas as ROMs compatíveis."""
        if not candidates:
...
=======
    ...
    def write_manifest(self, xml_machines: list[dict], xml_path: Path, output_path: Path, mame_version: str, source_paths: Iterable[Path | str]) -> Path:
        """Gera current_scan.jsonl com ROMs e CHDs explícitos."""
        if not self.last_scan_id: raise RuntimeError("Nenhum scan físico foi executado.")
        output_path.parent.mkdir(parents=True, exist_ok=True)
        # A GUI antiga ainda não fornece disks. Nesse caso o catálogo do banco
        # continua sendo a fonte de verdade dos CHDs.
        disks_by_machine = self._expected_disks or self._build_expected_disks([m["name"] for m in xml_machines])
        header = {"record_type": "header", "schema_version": 3, "scan_id": f"physical_{self.last_scan_id}", "started_at": datetime.now(timezone.utc).isoformat(), "mame_version": mame_version, "xml_path": str(xml_path), "source_paths": [str(Path(p)) for p in source_paths], "machine_count_expected": len(xml_machines), "metadata": {"validation": "physical_stream_crc32_sha1_size", "bytes_read": self.last_stats.get("bytes_read", 0), "chds_scanned": self.last_stats.get("chds", 0), "chds_valid": self.last_stats.get("chds_valid", 0)}}
        with output_path.open("w", encoding="utf-8", newline="\n") as handle:
            handle.write(json.dumps(header, ensure_ascii=False) + "\n")
...
>>>>>>> 5bc4f0bc4515edb1a835cd8055da8b3eccc6faac
    ...
    def _latest_rom_match(self, machine_name: str, rom_name: str):
        ...
        return self._connection().execute("""SELECT s.source_path, s.archive_member, s.source_kind, s.actual_size, s.actual_crc, s.actual_sha1, s.validation_status, s.error FROM rom_source_match s JOIN rom r ON r.id=s.rom_id JOIN machine m ON m.id=r.machine_id WHERE s.scan_run_id=? AND m.name=? AND r.name=? ORDER BY CASE s.validation_status WHEN 'valid' THEN 0 WHEN 'sha1_mismatch' THEN 1 ELSE 2 END, s.id DESC LIMIT 1""", (self.last_scan_id, machine_name, rom_name)).fetchone()
...
=======
        ...
        conn.execute("UPDATE rom_scan_run SET finished_at=CURRENT_TIMESTAMP, status=?, archive_count=?, member_count=?, loose_file_count=?, bytes_read=?, valid_match_count=?, unmatched_count=?, error=? WHERE id=?", (stats["status"], stats["archives"], stats["members"], stats["loose"], stats["bytes_read"], stats["valid"], stats["unmatched"] + stats["sha1_mismatch"] + stats["read_errors"] + stats.get("chds_missing", 0) + stats.get("chds_invalid", 0) + stats.get("chds_errors", 0), error, scan_id)); conn.commit()
...
=======
        ...
        return f"{path.name} | membros {stats['members']:,} | válidas {stats['valid']:,} | CHDs {stats.get('chds_valid', 0):,}/{stats.get('chds', 0):,} válidos | SHA1 divergente {stats['sha1_mismatch']:,} | não correspondentes {stats['unmatched']:,} | erros {stats['read_errors']:,} | bytes lidos {stats['bytes_read']:,}"
>>>>>>> 5bc4f0bc4515edb1a835cd8055da8b3eccc6faac
