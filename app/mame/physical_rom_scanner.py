...
class PhysicalRomScanner:
    ...
    def _normalize_paths(paths: Iterable[Path | str]) -> list[Path]:
        """Normaliza e elimina origens duplicadas."""
        result: list[Path] = []
        seen: set[str] = set()
        for value in paths:
    ...
    def scan(
        ...
        *,
        ...
        """Executa o scan e persiste SQLite/JSONL enquanto as machines terminam.

        O manifesto temporário é aberto antes da criação dos workers. Assim,
        a GUI não precisa fazer uma segunda passagem pelo conjunto para gerar
        o JSONL depois do scan.
        """
        ...
        stats["expected_roms"] = sum(len(items) for items in self._expected_roms.values())
        stats["expected_chds"] = sum(len(items) for items in self._expected_disks.values())
        ...
        logger.info(
            "Scan esperado iniciado: %d ROMs, %d CHDs, %d machines, %d workers.",
            stats["expected_roms"], stats["expected_chds"], len(names), self.max_workers,
        )
        if progress:
            progress(
                0,
                f"Preparando scan | {len(names):,} machines | "
                f"{stats['expected_roms']:,} ROMs | {stats['expected_chds']:,} CHDs",
            )
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers, thread_name_prefix="rom-scan") as executor:
                ...
                for future in as_completed(futures):
                    ...
                    message = self._progress_message(stats, machine_name, completed, len(names))
                    logger.info(message)
                    if progress:
                        progress(
                            min(100, completed * 100 // max(1, len(names))),
                            message,
                        )
            ...
            summary = self._summary_message(stats)
            logger.info(summary)
            if progress:
            ...
            try:
                manifest.write(
                    json.dumps(
                        {"record_type": "scan_end", "status": stats["status"], "error": str(exc)},
                        ensure_ascii=False,
                    ) + "\n"
                )
                manifest.flush()
    ...
    def _scan_machine(self, machine_name: str, scan_id: int, cancelled: Callable[[], bool] | None) -> dict:
        ...
        for candidate in expected:
            if candidate["rom_id"] not in found:
                unit["records"].append(
                    (
    ...
    def _build_expected_roms(self, names: list[str]) -> dict[str, list[dict]]:
        """Carrega size, CRC, SHA1 e metadados completos por machine."""
        result = {name: [] for name in names}
        if not names:
            return result
        placeholders = ",".join("?" for _ in names)
        rows = self.db.fetchall(
            f"""SELECT m.name machine_name, r.id, r.machine_id, r.name, r.size,
                       r.crc, r.sha1, r.merge, r.optional
    ...
    def _build_expected_disks(self, names: list[str]) -> dict[str, list[dict]]:
        ...
        placeholders = ",".join("?" for _ in names)
        rows = self.db.fetchall(
            f"""SELECT m.name machine_name, d.name, d.sha1, d.merge, d.region,
    ...
    def _scan_expected_zip(self, path: Path, expected: list[dict], unit: dict, cancelled: Callable[[], bool] | None) -> None:
        """Consulta somente membros esperados usando o diretório central ZIP."""
        try:
            with zipfile.ZipFile(path, "r") as archive:
                ...
                for candidate in expected:
                    ...
                    unit["records"].append(
                        (candidate["rom_id"], str(path), info.filename, "zip", size,
                         crc, actual_sha1, status, bytes_read, error)
                    )
    ...
    def _scan_expected_loose_dir(self, machine_dir: Path, expected: list[dict], unit: dict, cancelled: Callable[[], bool] | None) -> None:
        ...
        for candidate in expected:
            ...
            try:
                ...
                unit["records"].append(
                    (candidate["rom_id"], str(path), None, "loose", size, crc,
                     actual_sha1, status, size, None)
                )
            except (OSError, RuntimeError) as exc:
                unit["read_errors"] += 1
                unit["records"].append(
                    (candidate["rom_id"], str(path), None, "loose", 0, "", None,
                     "read_error", 0, str(exc))
                )
    ...
    def _record(self, conn: sqlite3.Connection, scan_id: int, rom_id: int | None, source_path: str,
                archive_member: str | None, source_kind: str, size: int, crc: str,
                sha1: str | None, status: str, bytes_read: int, error: str | None) -> None:
        conn.execute(
            """INSERT INTO rom_source_match
            (dataset_run_id, scan_run_id, rom_id, source_path, archive_member, source_kind,
             actual_size, actual_crc, actual_sha1, validation_status, bytes_read, checked_at, error)
            SELECT dataset_run_id, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?
            FROM rom_scan_run WHERE id=?""",
            (scan_id, rom_id, source_path or "", archive_member, source_kind, size, crc,
             sha1, status, bytes_read, error, scan_id),
        )
    ...
    def _write_unit_manifest(self, handle, unit: dict) -> None:
        ...
        expected = {item["rom_id"]: item for item in self._expected_roms.get(machine, [])}
        best: dict[int, tuple] = {}
        for row in unit["records"]:
            if row[0] is None:
                continue
            if row[0] not in best or self._status_rank(row[7]) < self._status_rank(best[row[0]][7]):
                best[row[0]] = row
        ...
        for rom_id, candidate in expected.items():
            row = best.get(rom_id)
            if row:
                _, source, member, kind, size, crc, sha1, status, _read, error = row
            else:
                source = member = sha1 = error = None
                kind, size, crc, status = "expected", 0, "", "missing"
            record = {
                "machine": machine, "rom_name": candidate["name"],
                "expected_size": candidate["size"], "expected_crc": candidate["crc"],
                "expected_sha1": candidate["sha1"] or None, "merge": candidate["merge"],
                "required": not candidate["optional"], "optional": candidate["optional"],
                "status": status, "actual_size": size, "actual_crc": crc or None,
                "actual_sha1": sha1,
                "source": {"kind": kind, "archive": source, "member": member, "machine": machine} if source else None,
                "error": error,
            }
        ...
        for disk_name, disk in unit["chd_results"].items():
            definition = next(
                (item for item in self._expected_disks.get(machine, []) if str(item.get("name") or "") == disk_name),
                {},
            )
            record = {
                "machine": machine, "disk_name": disk_name,
                "expected_size": int(definition.get("size") or 0),
                "expected_sha1": str(definition.get("sha1") or "").lower(),
                "required": not bool(definition.get("optional")),
                "optional": bool(definition.get("optional")),
                "status": disk["status"], "actual_size": 0, "actual_sha1": None,
    ...
    def _finish_manifest(self, handle, stats: dict) -> None:
        """Grava o resumo e fecha o manifesto streaming."""
    ...
    def _build_expected_index(self, machine_names=None):
        """Compatibilidade para consumidores antigos; o scanner não o usa para buscar o HDD."""
        result: dict[tuple[str, int], list[dict]] = {}
        selected = set(machine_names or self._expected_roms)
        for machine in selected:
            for row in self._expected_roms.get(machine, []):
                result.setdefault((row["crc"], row["size"]), []).append(row)
        return result
    ...
    def _new_stats(scan_id: int, run_id: int | None, names: list[str]) -> dict:
        return {
            ...
            "chds_valid": 0, "bytes_read": 0, "valid": 0, "missing": 0,
    ...
    def _update_stats(stats: dict, unit: dict) -> None:
        for key in (
            "archives", "members", "loose", "bytes_read", "valid", "missing",
            "sha1_mismatch", "unmatched", "read_errors", "chds", "chds_present",
            "chds_missing", "chds_errors",
        ):
    ...
    def _finish_run(self, conn: sqlite3.Connection, scan_id: int, stats: dict, error: str | None = None) -> None:
        conn.execute(
            """UPDATE rom_scan_run SET finished_at=CURRENT_TIMESTAMP, status=?, archive_count=?,
               member_count=?, loose_file_count=?, bytes_read=?, valid_match_count=?,
               unmatched_count=?, error=? WHERE id=?""",
            (
                stats["status"], stats["archives"], stats["members"], stats["loose"],
                stats["bytes_read"], stats["valid"],
                stats["unmatched"] + stats["missing"] + stats["sha1_mismatch"] + stats["read_errors"] + stats.get("chds_missing", 0) + stats.get("chds_errors", 0),
                error, scan_id,
            ),
        )
        conn.commit()
