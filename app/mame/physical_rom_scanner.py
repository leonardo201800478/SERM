...
class PhysicalRomScanner:
    ...
    def scan(self, machine_names: Iterable[str] | None = None, run_id: int | None = None,
             ...
             mame_version: str = "unknown") -> dict:
        """Executa o scan e persiste resultados enquanto as machines terminam.

        Quando a GUI não fornece ``manifest_path``, o manifesto temporário
        canônico ``current_scan.jsonl.tmp`` é aberto imediatamente. Assim a
        geração do JSONL não é uma segunda etapa depois do scan.
        """
        ...
        conn.execute("INSERT INTO rom_scan_run (dataset_run_id, source_count, status) VALUES (?, ?, 'running')",
        ...
        self._manifest_path = Path(manifest_path or "data/database/scan/current_scan.jsonl.tmp")
        self._manifest_path.parent.mkdir(parents=True, exist_ok=True)
        manifest = self._open_manifest(self._manifest_path, names, xml_path, mame_version)

        logger.info("Scan físico esperado iniciado: %d ROMs e %d CHDs em %d machine(s).",
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers, thread_name_prefix="rom-scan") as executor:
                futures = {executor.submit(self._scan_machine, name, scan_id, cancelled): name for name in names}
                completed = 0
                for future in as_completed(futures):
                    ...
                    completed += 1
                    if progress:
                        progress(min(100, completed * 100 // max(1, len(names))),
            ...
            logger.info(self._summary_message(stats))
            return stats
            ...
            try:
                ...
                logger.exception("Falha finalizando manifesto parcial.")
            self.last_stats = stats
    ...
    def _scan_machine(self, machine_name: str, scan_id: int, cancelled: Callable[[], bool] | None) -> dict:
        ...
        found_ids = {int(row[0]) for row in unit["records"] if row[0] is not None}
        for candidate in expected:
            if candidate["rom_id"] not in found_ids:
                unit["records"].append((candidate["rom_id"], "", None, "expected", 0, "", None, "missing", 0,
                                         "ROM não encontrada em machine.zip nem na pasta da machine"))
                unit["missing"] += 1
        return unit

    def _build_expected_roms(self, names: list[str]) -> dict[str, list[dict]]:
        """Carrega requisitos completos por machine, incluindo size/CRC/SHA1."""
        result = {name: [] for name in names}
        if not names: return result
        placeholders = ",".join("?" for _ in names)
        rows = self.db.fetchall(
            f"""SELECT m.name machine_name, r.id, r.machine_id, r.name, r.size, r.crc, r.sha1, r.merge, r.optional
                FROM rom r JOIN machine m ON m.id=r.machine_id
                WHERE m.name IN ({placeholders}) ORDER BY m.name, r.id""", names)
        for row in rows:
            result.setdefault(str(row["machine_name"]), []).append({
                "rom_id": int(row["id"]), "machine_id": int(row["machine_id"]), "machine_name": str(row["machine_name"]),
                "name": str(row["name"] or "").replace("\\", "/"), "size": int(row["size"] or 0),
                "crc": str(row["crc"] or "").strip().lower(), "sha1": str(row["sha1"] or "").strip().lower(),
                "merge": row["merge"], "optional": bool(row["optional"]),
            })
        return result

    def _build_expected_disks(self, names: list[str]) -> dict[str, list[dict]]:
        """Carrega apenas os CHDs declarados pelas machines selecionadas."""
        result = {name: [] for name in names}
        if not names: return result
        placeholders = ",".join("?" for _ in names)
        rows = self.db.fetchall(
            f"""SELECT m.name machine_name, d.name, d.sha1, d.merge, d.region, d.disk_index, d.writable, d.optional, d.size
                FROM disk d JOIN machine m ON m.id=d.machine_id
                WHERE m.name IN ({placeholders}) ORDER BY m.name, d.disk_index, d.id""", names)
        for row in rows:
            result.setdefault(str(row["machine_name"]), []).append({
                "name": str(row["name"] or ""), "sha1": str(row["sha1"] or "").strip().lower(), "merge": row["merge"],
                "region": row["region"], "index": int(row["disk_index"] or 0), "writable": bool(row["writable"]),
                "optional": bool(row["optional"]), "size": int(row["size"] or 0),
            })
        return result

    def _scan_expected_zip(self, path: Path, expected: list[dict], unit: dict, cancelled: Callable[[], bool] | None) -> None:
        """Consulta somente membros esperados via diretório central do ZIP."""
        try:
            with zipfile.ZipFile(path, "r") as archive:
                infos = {i.filename.replace("\\", "/"): i for i in archive.infolist() if not i.is_dir()}
                basenames = {Path(n).name: i for n, i in infos.items()}
                for candidate in expected:
                    self._check_cancelled(cancelled)
                    info = infos.get(candidate["name"]) or basenames.get(Path(candidate["name"]).name)
                    if info is None: continue
                    unit["members"] += 1
                    size = int(info.file_size); crc = f"{info.CRC & 0xFFFFFFFF:08x}"
                    ...
                    if status == "valid" and candidate["sha1"]:
                        try:
                            ...
                            status = "read_error"; error = str(exc); unit["read_errors"] += 1
            ...
            unit["read_errors"] += 1; unit["errors"].append(f"{path}: {exc}")

    def _scan_expected_loose_dir(self, machine_dir: Path, expected: list[dict], unit: dict, cancelled: Callable[[], bool] | None) -> None:
        """Consulta somente arquivos ROM esperados na pasta da machine."""
        for candidate in expected:
            self._check_cancelled(cancelled)
            path = machine_dir / Path(candidate["name"]).name
            if not path.is_file(): continue
            try:
                size = path.stat().st_size; crc = self._crc_file(path, cancelled)
                size_ok = size == candidate["size"] if candidate["size"] > 0 else True
                crc_ok = crc == candidate["crc"] if candidate["crc"] else True
                status = "valid" if size_ok and crc_ok else "invalid"
                actual_sha1 = None
                ...
                unit["loose"] += 1; unit["bytes_read"] += size
                unit["records"].append((candidate["rom_id"], str(path), None, "loose", size, crc, actual_sha1, status, size, None))
    ...
    def _scan_expected_chd(self, machine_name: str, disk: dict, cancelled: Callable[[], bool] | None) -> dict:
        """Testa apenas ``<machine>/<disk>.chd``; não lê o CHD."""
    ...
    def _persist_unit(self, conn: sqlite3.Connection, unit: dict) -> None:
        """Persiste uma machine em uma transação curta."""
        for row in unit["records"]: self._record(conn, unit["scan_id"], *row)
        conn.commit()

    def _record(self, conn: sqlite3.Connection, scan_id: int, rom_id: int | None, source_path: str, archive_member: str | None,
                source_kind: str, size: int, crc: str, sha1: str | None, status: str, bytes_read: int, error: str | None) -> None:
        conn.execute(
            """INSERT INTO rom_source_match
            (dataset_run_id, scan_run_id, rom_id, source_path, archive_member, source_kind, actual_size, actual_crc, actual_sha1, validation_status, bytes_read, checked_at, error)
    ...
    def _open_manifest(self, path: Path, names: list[str], xml_path: Path | str | None, mame_version: str):
        """Abre o JSONL imediatamente, antes dos workers iniciarem."""
        handle = path.open("w", encoding="utf-8", newline="\n")
        header = {"record_type": "header", "schema_version": 4, "scan_id": f"physical_{self.last_scan_id}",
                  "started_at": datetime.now(timezone.utc).isoformat(), "mame_version": mame_version,
                  "xml_path": str(xml_path) if xml_path else "", "source_paths": [str(p) for p in self.source_dirs],
                  "machine_count_expected": len(names), "metadata": {"validation": "expected_driven", "persist_mode": "streaming"}}
        handle.write(json.dumps(header, ensure_ascii=False) + "\n"); handle.flush()
        return handle

    def _write_unit_manifest(self, handle, unit: dict) -> None:
        """Escreve ROMs/CHDs de uma machine assim que ela termina."""
        ...
        for row in unit["records"]:
            ...
            old = best.get(row[0])
            if old is None or self._status_rank(row[7]) < self._status_rank(old[7]): best[row[0]] = row
        ...
        for rom_id, candidate in expected.items():
            row = best.get(rom_id)
            if row: _, source, member, kind, size, crc, sha1, status, _bytes, error = row
            else: source = member = sha1 = error = None; kind, size, crc, status = "expected", 0, "", "missing"
            record = {"machine": machine, "rom_name": candidate["name"], "expected_size": candidate["size"], "expected_crc": candidate["crc"],
                      "expected_sha1": candidate["sha1"] or None, "merge": candidate["merge"], "required": not candidate["optional"],
                      "optional": candidate["optional"], "status": status, "actual_size": size, "actual_crc": crc or None,
                      "actual_sha1": sha1, "source": {"kind": kind, "archive": source, "member": member, "machine": machine} if source else None, "error": error}
            handle.write(json.dumps({"record_type": "rom", "record": record}, ensure_ascii=False) + "\n")
        for disk_name, disk in unit["chd_results"].items():
            definition = next((d for d in self._expected_disks.get(machine, []) if str(d.get("name") or "") == disk_name), {})
            record = {"machine": machine, "disk_name": disk_name, "expected_size": int(definition.get("size") or 0),
                      "expected_sha1": str(definition.get("sha1") or "").lower(), "required": not bool(definition.get("optional")),
                      "optional": bool(definition.get("optional")), "status": disk["status"], "actual_size": 0,
                      "actual_sha1": None, "source": {"kind": "chd", "archive": disk.get("source_path"), "member": None, "machine": machine} if disk.get("source_path") else None,
    ...
    def write_manifest(self, xml_machines: list[dict], xml_path: Path, output_path: Path, mame_version: str, source_paths: Iterable[Path | str]) -> Path:
        """Mantém compatibilidade com a GUI antiga: o JSONL já foi escrito em streaming."""
    ...
    def _build_expected_index(self, machine_names: Iterable[str] | None) -> dict[tuple[str, int], list[dict]]:
        """Compatibilidade com chamadas antigas; requisitos reais são agrupados por machine."""
        result: dict[tuple[str, int], list[dict]] = {}
        for machine, rows in self._expected_roms.items():
            if machine_names is not None and machine not in set(machine_names): continue
            for row in rows: result.setdefault((row["crc"], row["size"]), []).append(row)
        return result

    @staticmethod
    def _new_stats(scan_id: int, run_id: int | None, names: list[str]) -> dict:
        return {"scan_id": scan_id, "dataset_run_id": run_id, "expected_roms": 0, "expected_chds": 0,
                "archives": 0, "members": 0, "loose": 0, "chds": 0, "chds_present": 0, "chds_missing": 0,
                "chds_errors": 0, "chds_valid": 0, "bytes_read": 0, "valid": 0, "missing": 0, "sha1_mismatch": 0,
                "unmatched": 0, "read_errors": 0, "seconds": 0.0, "status": "running", "machines": len(names), "machines_completed": 0}

    def _update_stats(self, stats: dict, unit: dict) -> None:
    ...
    def _progress_message(self, stats: dict, machine: str, completed: int, total: int) -> str:
        return (f"Machine {completed:,}/{total:,}: {machine} | ROMs {stats['members']:,} verificadas | válidas {stats['valid']:,} | "
    ...
    def _summary_message(stats: dict) -> str:
        return (f"Scan concluído: {stats['machines_completed']:,}/{stats['machines']:,} machines | ROMs válidas {stats['valid']:,} | "
                f"ausentes {stats['missing']:,} | CHDs presentes {stats['chds_present']:,}/{stats['chds']:,} | tempo {stats['seconds']:.2f}s")

    def _finish_run(self, conn: sqlite3.Connection, scan_id: int, stats: dict, error: str | None = None) -> None:
        conn.execute("""UPDATE rom_scan_run SET finished_at=CURRENT_TIMESTAMP, status=?, archive_count=?, member_count=?, loose_file_count=?, bytes_read=?, valid_match_count=?, unmatched_count=?, error=? WHERE id=?""",
                     (stats["status"], stats["archives"], stats["members"], stats["loose"], stats["bytes_read"], stats["valid"],
                      stats["unmatched"] + stats["missing"] + stats["sha1_mismatch"] + stats["read_errors"] + stats.get("chds_missing", 0) + stats.get("chds_errors", 0), error, scan_id))
        conn.commit()

    def _ensure_scan_tables(self, conn: sqlite3.Connection) -> None:
        conn.executescript(self._SCAN_TABLE_SQL); conn.commit()
