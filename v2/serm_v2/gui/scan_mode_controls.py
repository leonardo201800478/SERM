...
class ScanModeControls(QWidget):
    ...
    def __init__(self,parent=None):
        ...
        row=QHBoxLayout(self);row.addWidget(self.resume_button);row.addWidget(self.new_button);row.addWidget(self.status);row.addStretch()
        self.resume_button.clicked.connect(lambda:self._set(True));self.new_button.clicked.connect(lambda:self._set(False));self._set(True)
    def _set(self,resume):
        ...
        self.resume_button.setEnabled(not resume);self.new_button.setEnabled(resume)
