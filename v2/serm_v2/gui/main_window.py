...
class MainWindow(QMainWindow):
    NAV_ITEMS = (
        ...
        (
            "Ferramentas",
            "LaunchBox, 7-Zip e outros executáveis auxiliares",
            "SP_ComputerIcon",
        ),
        (
            "Configurações",
        ...
        (
            "MAME — Scans",
            "Novo scan, histórico e exclusão de scans MAME",
            "SP_DriveHDIcon",
        ),
        (
            "2 — Filtragem",
        ...
        (
            "MAME — Filtros",
            "Filtros MAME separados por tipo de jogo e tipo de SET",
            "SP_FileDialogDetailedView",
        ),
        (
            "No-Intro — Filtros",
            "Conteúdo, regiões, clones, hacks, traduções e 1G1R",
            "SP_FileDialogDetailedView",
        ),
        (
            "3 — Reconstrução",
            "Montar o set a partir do arquivo filtrado",
    ...
    def _find_restore_screen(self, screens, saved_screen, rect):
        target = next(
            (screen for screen in screens if self._get_screen_key(screen) == saved_screen),
            None,
        )
        if target is None and saved_screen:
            name = saved_screen.split("|", 1)[0]
            target = next(
                (screen for screen in screens if screen.name().strip() == name),
                None,
            )
    ...
    def _restore_window_layout(self) -> None:
        ...
        if not screens:
            return
        target = self._find_restore_screen(
            screens,
            saved_screen,
            self.frameGeometry(),
        )
