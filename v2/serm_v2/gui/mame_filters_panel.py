...
class MameFiltersPanel(QWidget):
    ...
    def _essential_tab(self) -> QWidget:
        ...
        hint = QLabel("Marcado = excluir. O resultado é calculado pelo motor V2 após cada alteração.")
    ...
    def _worker_ready(self, operation: str, payload) -> None:
        if operation == "facets":
            ...
            self.scan_info.setText(f"{self._scan_path}\nOpções descobertas no snapshot. Filtros avançados são dados reais, não uma lista fixa da V1.")
    ...
    def _populate_facets(self, facets: dict[str, list[dict[str, object]]]) -> None:
        for key, widget in self._facet_widgets.items():
            ...
            if info:
                info.setText(f"{len(values):,} opções encontradas. Seleção múltipla usa OR nesta dimensão e AND entre dimensões.")
    ...
    def _format_result(payload: dict, prefix: str) -> str:
        ...
        return (
            ...
            f"Pipeline: " + " | ".join(f"{key}={value:,}" for key, value in stages.items()) + "\n"
