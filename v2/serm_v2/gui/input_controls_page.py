...
class InputControlsPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QLabel("Hardware detectado pelo SERM • sem encaminhamento de eventos")
        ...
        self.grid.setContentsMargins(0, 0, 0, 0); self.grid.setHorizontalSpacing(8); self.grid.setVerticalSpacing(8)
    ...
    def _apply_style(self) -> None:
        self.setStyleSheet(
            ...
            "QLabel#controlsTitle{color:#e7eef7;font-size:14px;font-weight:650;}"
            "QLabel#controlsSubtitle{color:#8fa5bb;font-size:8pt;}"
            "QLabel#controlsStatus{color:#8fa5bb;font-size:8pt;padding:2px 4px;}"
            "QPushButton#controlsRefresh{background:#172b42;color:#eaf5ff;border:1px solid #3e6d8b;border-radius:6px;padding:6px 11px;}"
            "QPushButton#controlsRefresh:hover{background:#1d3854;}"
            "QFrame#deviceCard{background:#0d1726;border:1px solid #293b54;border-radius:9px;}"
            "QLabel#deviceName{color:#e8f0f8;font-size:11pt;font-weight:650;}"
            "QLabel#deviceMeta{color:#8fa5bb;font-size:8pt;}"
            "QLabel#deviceGood{color:#78d6b0;font-size:8pt;font-weight:600;}"
            "QLabel#deviceWarn{color:#e0c477;font-size:8pt;font-weight:600;}")
    ...
    def _card(self, snapshot) -> QFrame:
        ...
        name = QLabel(device.name or "Dispositivo sem nome"); name.setObjectName("deviceName")
        box.addWidget(name)
        ...
        meta = QLabel(f"{kind}  •  {connection}  •  VID {device.vendor_id or 0:04X} / PID {device.product_id or 0:04X}")
        meta.setObjectName("deviceMeta"); box.addWidget(meta)
        ...
        buttons = getattr(layout, "button_count", 0)
        axes = getattr(layout, "axis_count", 0)
        hats = getattr(layout, "hat_count", 0)
        box.addWidget(QLabel(f"Layout: {profile}  •  {buttons} botões  •  {axes} eixos  •  {hats} hats"))
        if getattr(layout, "has_six_face_buttons", False):
            label = QLabel("✓ Seis botões de face detectados")
            label.setObjectName("deviceGood"); box.addWidget(label)
        if snapshot.identification.model is not None:
            label = QLabel(f"✓ Modelo: {snapshot.identification.model.model_name} ({snapshot.identification.confidence}%)")
            label.setObjectName("deviceGood"); box.addWidget(label)
        else:
    ...
    def refresh(self) -> None:
        ...
        try:
            ...
            self.status.setText(f"Falha na detecção: {exc}")
            return
        ...
        self.status.setText(f"{count} dispositivo(s) físico(s) detectado(s). A leitura foi realizada somente para diagnóstico.")
