...
from PySide6.QtWidgets import QCheckBox, QComboBox, QFileDialog, QGroupBox, QHBoxLayout, QLabel, QListWidget, QMessageBox, QPushButton, QScrollArea, QSplitter, QTreeWidget, QTreeWidgetItem, QVBoxLayout, QWidget
...
class FilterProfilesPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("SERM V2 — Filtros e Scan"); title.setProperty("role", "title"); root.addWidget(title)
        intro = QLabel("O perfil define o SET desejado. Salve o perfil antes do scan; o mesmo perfil será reutilizado pelo scanner e pela reconstrução."); intro.setWordWrap(True); root.addWidget(intro)
        splitter = QSplitter(Qt.Orientation.Horizontal); splitter.addWidget(self._source_tree()); splitter.addWidget(self._editor()); splitter.setStretchFactor(0, 0); splitter.setStretchFactor(1, 1); root.addWidget(splitter, 1)

    def _source_tree(self) -> QWidget:
        box = QGroupBox("Catálogos e perfis"); layout = QVBoxLayout(box)
        self.source_tree = QTreeWidget(); self.source_tree.setHeaderLabels(["Fonte / sistema"]); self.source_tree.setMinimumWidth(280); self.source_tree.itemSelectionChanged.connect(self._selection_changed); layout.addWidget(self.source_tree, 1)
    ...
    def _editor(self) -> QWidget:
        ...
        source_box = QGroupBox("Fontes temporárias do scan — máximo 3"); source_layout = QVBoxLayout(source_box); self.source_list = QListWidget(); source_layout.addWidget(self.source_list)
        ...
        scroll = QScrollArea(); scroll.setWidgetResizable(True); scroll.setFrameShape(QScrollArea.Shape.NoFrame); body = QWidget(); self.filter_layout = QVBoxLayout(body); self._build_generic_controls(); self._build_mame_controls(); self.filter_layout.addStretch(); scroll.setWidget(body); layout.addWidget(scroll, 1)
    ...
    def _build_mame_controls(self) -> None:
        self.mame_box = QGroupBox("MAME — filtro baseado na V1"); layout = QVBoxLayout(self.mame_box); layout.addWidget(QLabel("Tipo de SET:")); self.mame_set_type = QComboBox(); self.mame_set_type.addItem("Split — padrão", "split"); self.mame_set_type.addItem("Non-Merged", "non_merged"); self.mame_set_type.addItem("Full-Merged", "full_merged"); layout.addWidget(self.mame_set_type); layout.addWidget(QLabel("Clones:")); self.mame_clone_policy = QComboBox(); self.mame_clone_policy.addItem("Com clones", "with_clones"); self.mame_clone_policy.addItem("Somente parents", "parents_only"); layout.addWidget(self.mame_clone_policy)
        ...
        layout.addWidget(QLabel("Classificação: catlist.ini tem precedência; ListXML permanece preservado.")); self.mame_classification = QComboBox(); self.mame_classification.addItem("catlist.ini", "catlist"); self.mame_classification.addItem("ListXML", "listxml"); layout.addWidget(self.mame_classification); self.filter_layout.addWidget(self.mame_box)
    ...
    def _selection_changed(self) -> None:
        ...
        self.selected_label.setText(f"{source}  ›  {system}"); self._selected_dat = Path(dat_path) if dat_path else None
