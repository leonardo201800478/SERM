...
class ToolsDirectoriesPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        browse = QPushButton("Selecionar LaunchBox.exe"); browse.clicked.connect(self.select_launchbox)
        ...
        browse7 = QPushButton("Selecionar 7z.exe"); browse7.clicked.connect(self.select_7zip)
        ...
        actions = QHBoxLayout()
        refresh = QPushButton("Redetectar"); refresh.clicked.connect(self.refresh); actions.addWidget(refresh)
        save = QPushButton("Salvar"); save.clicked.connect(self.save); actions.addWidget(save)
    ...
    def _load_tools(self) -> dict:
        ...
        try:
            return json.loads(self.CONFIG_PATH.read_text(encoding="utf-8")) if self.CONFIG_PATH.is_file() else {}
    ...
    def refresh(self) -> None:
        ...
        self.launchbox_edit.setText(str(launchbox or configured.get("launchbox") or ""))
        ...
        detected = Path(sevenzip) if sevenzip and Path(sevenzip).is_file() else RetroArchDownloadService.detect_7zip()
    ...
    def select_launchbox(self) -> None:
        """Seleciona manualmente o executável do LaunchBox e persiste a integração original."""
    ...
    def select_7zip(self) -> None:
        ...
        if path:
            self.sevenzip_edit.setText(path); self.save()

    def save(self) -> None:
        """Persiste somente os caminhos auxiliares sem alterar o retroarch.cfg."""
