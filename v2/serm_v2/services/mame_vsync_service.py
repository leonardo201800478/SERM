...
class MameVsyncService:
    ...
    def locate_vsync_ini(self) -> Path:
        """Localiza Vsync.ini, aceitando apenas o nome lógico oficial."""
        for candidate in (self.mame_root / "folders" / "Vsync.ini", self.mame_root / "Vsync.ini"):
            if candidate.is_file():
                return candidate
        # Windows é case-insensitive, mas o teste acima mantém a convenção oficial.
    ...
    def _entries(path: Path):
        """Produz nomes de máquinas, ignorando comentários e seções INI."""
        with path.open("r", encoding="utf-8-sig", errors="replace") as handle:
            for raw_line in handle:
                line = raw_line.strip()
                if not line or line.startswith((";", "#")):
                    continue
                # Vsync.ini é uma lista plana; se houver cabeçalhos, eles não são máquinas.
                if line.startswith("[") and line.endswith("]"):
                    continue
                yield line

    def ingest(self, logger=None) -> dict[str, int | str]:
        """Importa Vsync.ini; SHA já concluído é reutilizado sem novo source_id."""
        ...
        try:
            ...
            if previous and previous[1] == "completed":
                ...
                row = connection.execute(
                    "SELECT COUNT(*), SUM(resolved_status='resolved'), SUM(resolved_status='unresolved') FROM mame_vsync WHERE source_document_id=?",
                    (source_id,),
                ).fetchone()
            ...
            for machine_name in self._entries(path):
                ...
                if machine_id is None:
                    ...
                    resolved += 1
                if entries % 5000 == 0:
                    log(f"MAME | VSYNC | PROGRESS | entradas={entries:,} | resolvidas={resolved:,} | não_resolvidas={unresolved:,}")
            connection.execute("UPDATE mame_source_document SET status='completed' WHERE id=?", (source_id,))
            connection.commit()
            log(f"MAME | VSYNC | DONE | entradas={entries:,} | resolvidas={resolved:,} | não_resolvidas={unresolved:,} | source_id={source_id}")
