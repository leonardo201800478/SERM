...
class MameFolderFilterService:
    ...
    def ingest(self, logger=None) -> dict[str, int | str]:
        ...
        total_entries = 0; imported_files = 0; connection = sqlite3.connect(self.database_path, timeout=120.0); connection.execute("PRAGMA foreign_keys=ON")
        try:
            for path in files:
                source_hash, byte_length = self._hash_file(path)
                if connection.execute("SELECT 1 FROM mame_folder_filter_source WHERE source_hash=? LIMIT 1", (source_hash,)).fetchone():
                ...
                if old_ids: connection.executemany("DELETE FROM mame_folder_filter_source WHERE id=?", [(source_id,) for source_id in old_ids])
                now = datetime.now(UTC).isoformat(); cursor = connection.execute("INSERT INTO mame_folder_filter_source (file_name,file_path,source_hash,byte_length,imported_at,status) VALUES(?,?,?,?,?,'captured')", (path.name, str(path), source_hash, byte_length, now)); source_id = int(cursor.lastrowid); entries = list(cls._entries(path))
    ...
    def machine_names_for_files(cls, database_path: Path, file_names: tuple[str, ...], machine_names: set[str] | None = None) -> set[str]:
        ...
        try:
            ...
            if not lowered: return set()
            file_placeholders = ",".join("?" for _ in lowered)
            rows = connection.execute(f"SELECT id FROM mame_folder_filter_source WHERE lower(file_name) IN ({file_placeholders})", tuple(sorted(lowered))).fetchall()
