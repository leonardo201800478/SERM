...
class MameFilterV2Service:
    ...
    O scan JSON limita o universo às machines efetivamente encontradas no
    snapshot físico. Nenhum atributo de filtro é inferido a partir da simples
    presença de uma ROM no JSON quando o banco possui a informação estrutural.
    """
    ...
    _FOLDER_FILES = {
        ...
        "clones": ("Clones Arcade.ini", "Clones Arcade.ini"),
        ...
        "game_or_no_game": ("Game Or No Game.ini", "[GAMING HISTORY] Game Or No Game.ini"),
    }
    ...
    def _import_id(cls, connection: sqlite3.Connection, payload: dict) -> tuple[int, str | None]:
        ...
        if source_hash:
            row = connection.execute(
                "SELECT id,mame_build FROM mame_listxml_import WHERE source_hash=? AND status='completed' ORDER BY id DESC LIMIT 1",
                (source_hash,),
            ).fetchone()
            if row:
        ...
        if build_match:
            row = connection.execute(
                "SELECT id,mame_build FROM mame_listxml_import WHERE mame_build LIKE ? AND status='completed' ORDER BY id DESC LIMIT 1",
                (f"%{build_match.group(1)}%",),
            ).fetchone()
            if row:
                return int(row[0]), row[1]
        row = connection.execute(
            "SELECT id,mame_build FROM mame_listxml_import WHERE status='completed' ORDER BY id DESC LIMIT 1"
        ).fetchone()
        if row:
            return int(row[0]), row[1]
    ...
    def _games(cls, payload: dict) -> list[ArcadeGame]:
        physical_names = {
            str(item.get("machine_name") or item.get("machine") or item.get("name") or "").strip()
        ...
        with sqlite3.connect(database_path(), timeout=60.0) as db:
            ...
            query = f"""
                SELECT m.id,m.name,m.cloneof,m.romof,m.isbios,m.isdevice,m.ismechanical,m.runnable,
                       m.description,m.year,m.manufacturer,m.sourcefile,
                       GROUP_CONCAT(DISTINCT c.category), GROUP_CONCAT(DISTINCT c.subcategory),
                       MAX(d.status), MAX(d.emulation), MAX(d.sound), MAX(d.graphic),
                       GROUP_CONCAT(DISTINCT disp.type), GROUP_CONCAT(DISTINCT disp.rotate),
                       GROUP_CONCAT(DISTINCT disp.width || 'x' || disp.height),
                       GROUP_CONCAT(DISTINCT disp.refresh_raw),
                       GROUP_CONCAT(DISTINCT ctl.type), MAX(ctl.buttons), MAX(inp.players),
                       GROUP_CONCAT(DISTINCT chip.type || ':' || COALESCE(chip.name,'')),
                       COUNT(DISTINCT disk.id), COUNT(DISTINCT sample.id), COUNT(DISTINCT bios.id),
                       COUNT(DISTINCT device.id), GROUP_CONCAT(DISTINCT rom.name),
                       GROUP_CONCAT(DISTINCT disk.name)
                FROM mame_machine m
            ...
            for row in rows:
                (
                    machine_id,name,cloneof,romof,isbios,isdevice,ismechanical,runnable,description,year,manufacturer,sourcefile,
                    categories,subcategories,driver_status,emulation,sound,graphic,display_types,rotates,resolutions,refreshes,
                    controls,buttons,players,chips,disk_count,sample_count,bios_count,device_count,rom_names,disk_names,
                ) = row
                category_values = [value.strip() for value in str(categories or "").split(",") if value.strip()]
                subcategory_values = [value.strip() for value in str(subcategories or "").split(",") if value.strip()]
                metadata = {
                ...
                games.append(
                    ArcadeGame(
                        machine_name=str(name), display_name=str(description or name), platform=ArcadePlatform.MAME,
                        parent_name=str(cloneof) if cloneof else None,
                        category=category_values[0] if category_values else None,
                        subcategory=subcategory_values[0] if subcategory_values else None,
                        metadata=metadata,
                    )
                )
            return games
    ...
    def _folder_maps(cls, db: sqlite3.Connection, machine_names: set[str]) -> dict[str, dict[str, set[str]]]:
        """Carrega, em poucas consultas, as listas auxiliares relevantes."""
        ...
        source_by_key: dict[str, set[int]] = {key: set() for key in cls._FOLDER_FILES}
        ...
        for key, ids in source_by_key.items():
            ...
            rows = db.execute(
                f"SELECT machine_name,COALESCE(section,'') FROM mame_folder_filter_entry WHERE source_id IN ({placeholders_ids}) AND machine_name IN ({placeholders_names})",
                (*sorted(ids), *sorted(machine_names)),
            ).fetchall()
            mapping = result[key]
    ...
    def _candidate_names(cls, games: list[ArcadeGame], state) -> set[str] | None:
        ...
        def match(game: ArcadeGame) -> bool:
            ...
            parent = str(game.parent_name or "").casefold()
