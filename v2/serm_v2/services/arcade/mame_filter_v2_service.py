...
class MameFilterV2Service:
    ...
    def _candidate_names(cls, games: list[ArcadeGame], state) -> set[str] | None:
        ...
        def match(game: ArcadeGame) -> bool:
            ...
            return True
        if any(queries.values()) or type_filter != "all" or year_from is not None or year_to is not None: candidate_sets.append({game.machine_name for game in games if match(game)})
