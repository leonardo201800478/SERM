...
class MameFilterV2Service:
    ...
    def _ensure_folder_filters(cls) -> None:
        ...
        try:
            with sqlite3.connect(database_path(), timeout=30.0) as db:
                if db.execute("SELECT COUNT(*) FROM mame_folder_filter_source").fetchone()[0]:
                    return
    ...
    def _candidate_names(cls, games: list[ArcadeGame], state) -> set[str] | None:
        ...
        def match(game: ArcadeGame) -> bool:
            ...
            year = cls._year(game)
            # O seletor de ano representa exclusivamente anos de machines Arcade,
            # mesmo quando MameCab only estiver desligado.
            if year_from is not None or year_to is not None:
                if not cls._is_arcade(game): return False
