...
class MameFilterV2Service:
    ...
    def _payload(cls, path: Path) -> dict:
        ...
        if str(payload.get("source", "")).casefold() != "mame": raise ValueError("O snapshot selecionado não é um scan MAME.")
        return payload
    ...
    def _games(cls, payload: dict) -> list[ArcadeGame]:
        ...
        with sqlite3.connect(database_path(), timeout=60.0) as db:
            ...
            for row in rows:
                ...
                # Em MAME, uma machine sem classificação de console/computer/etc. é
                # arcade por definição do filtro. O fallback não é aplicado a BIOS,
                # devices ou machines mecânicas.
                ...
                folder_filters = {key: sorted(values.get(name, set())) for key, values in folder_maps.items()}; genre_values = folder_filters.get("genre", []); series_values = folder_filters.get("series", [])
    ...
    def _folder_maps(cls, db: sqlite3.Connection, machine_names: set[str]) -> dict[str, dict[str, set[str]]]:
        result: dict[str, dict[str, set[str]]] = {key: {} for key in cls._FOLDER_FILES}
        if not machine_names: return result
        placeholders_names = ",".join("?" for _ in machine_names); source_rows = db.execute("SELECT id,file_name FROM mame_folder_filter_source").fetchall(); source_by_key = {key: set() for key in cls._FOLDER_FILES}
        for source_id, file_name in source_rows:
            lowered = str(file_name).casefold()
            for key, names in cls._FOLDER_FILES.items():
                if lowered in {name.casefold() for name in names}: source_by_key[key].add(int(source_id))
        for key, ids in source_by_key.items():
            if not ids: continue
            placeholders_ids = ",".join("?" for _ in ids); rows = db.execute(f"SELECT machine_name,COALESCE(section,'') FROM mame_folder_filter_entry WHERE source_id IN ({placeholders_ids}) AND machine_name IN ({placeholders_names})", (*sorted(ids), *sorted(machine_names))).fetchall(); mapping = result[key]
            for machine_name, section in rows: mapping.setdefault(str(machine_name), set()).add(str(section))
        return result

    @classmethod
    def facets(cls, path: Path) -> dict[str, list[dict[str, object]]]:
        games = cls._games(cls._payload(path)); classifier = ArcadeFilterEngine()._classifier; counters = {key: Counter() for key in ("year", "content", "playability", "genre", "hardware", "manufacturer", "series", "input", "wheel")}
        for game in games:
    ...
    def _year(game: ArcadeGame) -> int | None:
        match = re.search(r"(?:19|20)\d{2}", str(game.metadata.get("year") or "")); return int(match.group()) if match else None

    @staticmethod
    def _is_arcade(game: ArcadeGame) -> bool:
        if game.is_bios or game.is_device or bool(game.metadata.get("ismechanical")): return False
    ...
    def _is_horizontal(game: ArcadeGame) -> bool:
        rotate = str(game.metadata.get("rotate") or "").casefold(); display = str(game.metadata.get("display") or "").casefold()
        if any(value in rotate for value in ("90", "270", "vertical")): return False
        if "vertical" in display or "tate" in display: return False
        return True

    @staticmethod
    def _is_vertical(game: ArcadeGame) -> bool: return not MameFilterV2Service._is_horizontal(game)

    @classmethod
    def _candidate_names(cls, games: list[ArcadeGame], state) -> set[str] | None:
        candidate_sets: list[set[str]] = []; game_names = {game.machine_name for game in games}
        if state.categories or state.subcategories:
            names = MameCategoryFilterService.matching_machine_names({"categories": state.categories, "subcategories": state.subcategories}, database_path()); candidate_sets.append(names.intersection(game_names))
        queries = {key: str(getattr(state, key, "") or "").strip().casefold() for key in ("title_query", "full_text", "rom_query", "parent_query", "clone_query", "video_query", "audio_query", "screen_query", "cabinet_query", "channels_query")}; type_filter = str(getattr(state, "type_filter", "all") or "all"); year_from, year_to = getattr(state, "year_from", None), getattr(state, "year_to", None); orientation = str(getattr(state, "orientation", "all") or "all").casefold()
        def match(game: ArcadeGame) -> bool:
            ...
            return True
        if any(queries.values()) or type_filter != "all" or year_from is not None or year_to is not None or orientation != "all": candidate_sets.append({game.machine_name for game in games if match(game)})
    ...
    def preview(cls, path: Path, state) -> dict:
        _, _, result = cls._result(path, state); reasons = Counter(item.trace.rule or item.trace.stage.value for item in result.excluded); return {"unit": "machines", "input_count": result.total_input, "output_count": result.included_count, "filtered_count": result.excluded_count, "filter_counts": dict(reasons), "stage_counts": {stage.value: count for stage, count in result.counts_after_stage.items()}}

    @classmethod
    def apply(cls, path: Path, state) -> dict:
        payload, _, result = cls._result(path, state); kept_names = {item.game.machine_name for item in result.games}; evidence = list(payload.get("evidence", [])); kept_evidence = [item for item in evidence if str(item.get("machine_name") or item.get("machine") or item.get("name") or "") in kept_names]; reasons = Counter(item.trace.rule or item.trace.stage.value for item in result.excluded); run_id = uuid4().hex[:16]; out_dir = scans_root() / "filtered" / "mame"; out_dir.mkdir(parents=True, exist_ok=True); label = re.sub(r"[^A-Za-z0-9._-]+", "_", str(payload.get("catalog_label") or "catalog")); output_path = out_dir / f"MAME_{label}_{payload.get('scan_type', 'arcade')}_FILTER_{run_id}.json"
