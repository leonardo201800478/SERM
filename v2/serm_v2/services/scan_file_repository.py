...
class ScanFileRepository:
    ...
    def _mame_type_label(scan_type: str) -> str:
        return {"arcade": "Arcade", "software": "Software", "both": "Completa"}.get(
            str(scan_type).casefold(), str(scan_type)
        )
    ...
    def save(cls, result) -> Path:
        ...
        if stream_path and Path(stream_path).is_file():
            with path.open("w", encoding="utf-8", newline="\n") as output:
                prefix = json.dumps(
                    {k: v for k, v in header.items() if k != "evidence"},
                    ensure_ascii=False,
                    separators=(",", ":"),
                )
        ...
        with path.open("w", encoding="utf-8") as output:
            json.dump(
                header | {"evidence": [cls._evidence_payload(item) for item in result.evidence]},
                output,
                indent=2,
                ensure_ascii=False,
            )
