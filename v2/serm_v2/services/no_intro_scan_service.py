...
O DAT é a autoridade para identidade e hashes. O scan também interpreta a
convenção de nomes No-Intro para permitir filtros por BIOS, Program, Beta,
Proto, Demo, NP, região, idioma, clone e variantes externas de tradução/hack.
"""
...
class NoIntroScanService:
    ...
    VARIANT_PATTERNS = (
        ("translation", re.compile(r"(?:translation|translated|\[t\+|\[t-|\(t\+)", re.I)),
    ...
    TYPE_TOKENS = {"bios": "type:bios", "program": "type:program", "beta": "type:beta", "proto": "type:proto", "prototype": "type:proto", "demo": "type:demo", "sample": "type:sample", "np": "type:np", "aftermarket": "type:aftermarket", "unl": "type:unlicensed", "unlicensed": "type:unlicensed"}
    ...
    def scan(self, profile) -> ScanResult:
        ...
        total = len(expected); completed = 0
        ...
        with stream_path.open("w", encoding="utf-8", newline="\n") as stream:
            ...
            for game_name, items in games.items():
                ...
                completed += len(items)
                if self.progress_callback: self.progress_callback(completed, total)
                if completed == total or completed % 250 == 0: stream.flush()
    ...
    def _resolve_dat_path(profile) -> Path:
        ...
        if path is None or not path.is_file(): raise NoIntroScanError("O perfil No-Intro não possui um DAT local válido.")
        return path

    @staticmethod
    def _resolve_sources(profile) -> list[Path]:
        sources = [Path(p).expanduser().resolve() for p in profile.source_directories]
        if not sources: raise NoIntroScanError("Nenhum diretório de origem foi configurado para o scan No-Intro.")
        for source in sources:
            if not source.is_dir(): raise NoIntroScanError(f"Diretório de origem não encontrado: {source}")
        return sources

    def _scan_game(self, game_name, items, sources, result, stream) -> None:
        expected_by_name: dict[str, list[_ExpectedRom]] = {}
        for item in items: expected_by_name.setdefault(Path(item.rom_name).name.casefold(), []).append(item)
        occurrences: Counter[tuple[str,str,int,str]] = Counter()
        for source in sources:
            archive = source / f"{game_name}.zip"
            if archive.is_file():
                result.files_examined += 1; result.archives_examined += 1
                self._scan_zip(archive, expected_by_name, occurrences, result, stream)
            for item in items:
                loose = source / Path(item.rom_name).name
                if loose.is_file():
                    result.files_examined += 1; self._scan_loose(loose, item, occurrences, result, stream)
        for item in items:
            ...
            with zipfile.ZipFile(archive) as zf:
                for info in zf.infolist():
                    ...
                    for item in candidates:
                        key = self._item_key(item); duplicate = occurrences[key] > 0
                        evidence = self._validate_zip_member(zf, info, item, archive, duplicate=duplicate)
                        occurrences[key] += 1; self._emit(evidence, result, stream)
        except (OSError, zipfile.BadZipFile, RuntimeError) as exc:
            result.errors += 1; self._log_error(result, stream, archive, exc)

    def _scan_loose(self, path, item, occurrences, result, stream) -> None:
        key = self._item_key(item)
        try:
            size, crc, md5, sha1 = self._hash_file(path); status = self._compare(item, size, crc, md5, sha1)
            if status == "CURRENT" and occurrences[key] > 0: status = "DUPLICATE"
            occurrences[key] += 1
            ...
            result.errors += 1; self._log_error(result, stream, path, exc)
    ...
    def _validate_zip_member(self, zf, info, item, archive, *, duplicate: bool):
        ...
        if status == "CURRENT" and duplicate: status = "DUPLICATE"
    ...
    def _compare(item, size, crc, md5, sha1):
        if size != item.size or (item.crc and crc.casefold() != item.crc.casefold()) or (item.md5 and md5.casefold() != item.md5.casefold()) or (item.sha1 and sha1.casefold() != item.sha1.casefold()): return "WRONG"
        return "CURRENT"
    ...
    def _discover_unverified_variants(self, sources, known_bases, result, stream) -> None:
        for source in sources:
            for archive in source.glob("*.zip"):
                ...
                stem = archive.stem; variant = self._variant_kind(stem)
                ...
                try:
                    with zipfile.ZipFile(archive) as zf:
                        ...
                        for info in zf.infolist():
                            ...
                            self._emit(evidence, result, stream); result.items_examined += 1
                except (OSError, zipfile.BadZipFile, RuntimeError) as exc:
                    result.errors += 1; self._log_error(result, stream, archive, exc)

    def _match_variant_base(self, name, known_bases):
        normalized = self._normalize_name(name)
        if normalized in known_bases: return known_bases[normalized]
        ...
        for candidate, original in known_bases.items():
            ...
            if score > best_score: best_score, best_name = score, original
    ...
    @classmethod
    def _parse_name(cls, game_name, rom_name, cloneof):
        groups = re.findall(r"\(([^()]*)\)", game_name); tokens=[]
        for group in groups: tokens.extend([part.strip() for part in group.split(",") if part.strip()])
        regions=[]; languages=[]; tags=["clone:yes" if cloneof else "clone:no"]
        if game_name.startswith("[BIOS]"): tags.append("type:bios")
        for token in tokens:
            lower=token.casefold(); region=cls.REGION_ALIASES.get(lower)
            if region: regions.append(region)
            if token in cls.LANGUAGE_CODES: languages.append(token)
            normalized=lower.replace(" ", "")
            if normalized in cls.TYPE_TOKENS: tags.append(cls.TYPE_TOKENS[normalized])
            if lower.startswith("beta"): tags.append("type:beta")
            if lower.startswith("proto"): tags.append("type:proto")
            if lower.startswith("rev") or lower.startswith("v"): tags.append(f"version:{token}")
        extension=Path(rom_name).suffix.casefold().lstrip(".")
        if extension: tags.append(f"extension:{extension}")
        tags.extend(f"region:{r}" for r in dict.fromkeys(regions)); tags.extend(f"language:{l}" for l in dict.fromkeys(languages))
        base=re.sub(r"^\[BIOS\]\s*", "", game_name, flags=re.I); base=re.sub(r"\s+\([^()]*\)", "", base).strip()
    ...
    @classmethod
    def _name_metadata(cls, name): return cls._parse_name(name, name, None)

    @classmethod
    def _parse_game_roms(cls, game):
        game_name=str(game.attrib.get("name") or "").strip(); cloneof=str(game.attrib.get("cloneof") or "").strip() or None
        if not game_name: return []
        meta=cls._parse_name(game_name, "", cloneof); result=[]
        for rom in game.findall("rom"):
            rom_name=str(rom.attrib.get("name") or "").strip()
            if not rom_name: continue
            try: size=int(rom.attrib.get("size") or 0)
            except ValueError: size=0
            rom_meta=cls._parse_name(game_name, rom_name, cloneof); tags=tuple(dict.fromkeys((*meta["tags"],*rom_meta["tags"])))
            result.append(_ExpectedRom(game_name,rom_name,size,str(rom.attrib.get("crc") or "").casefold(),str(rom.attrib.get("md5") or "").casefold(),str(rom.attrib.get("sha1") or "").casefold(),str(rom.attrib.get("status") or "").casefold(),str(rom.attrib.get("format") or "").strip(),cloneof,meta["base_name"],meta["regions"],meta["languages"],tags))
        return result

    @classmethod
    def _load_dat(cls,path):
        try: root=ElementTree.parse(path).getroot()
        except (OSError,ElementTree.ParseError) as exc: raise NoIntroScanError(f"DAT No-Intro inválido: {path}: {exc}") from exc
        node=root.find("header"); header={c.tag:(c.text or "").strip() for c in node} if node is not None else {}
        return [item for game in root.findall(".//game") for item in cls._parse_game_roms(game)], header

    @staticmethod
    def _group_by_game(items):
        grouped={}
        for item in items: grouped.setdefault(item.game_name,[]).append(item)
        return grouped

    @staticmethod
    def _item_key(item): return (item.game_name.casefold(),item.rom_name.casefold(),item.size,item.sha1.casefold())
    ...
    def _hash_zip_member_full(cls,zf,info):
        crc=0; size=0; md5=hashlib.md5(usedforsecurity=False); sha1=hashlib.sha1(usedforsecurity=False)
        with zf.open(info,"r") as handle:
            for chunk in iter(lambda:handle.read(cls.CHUNK_SIZE),b""):
                size+=len(chunk); crc=zlib.crc32(chunk,crc); md5.update(chunk); sha1.update(chunk)
    ...
    def _normalize_name(value):
        value=re.sub(r"\.[A-Za-z0-9]{1,8}$","",str(value)); value=re.sub(r"\[[^]]+\]"," ",value); value=re.sub(r"\([^)]*\)"," ",value); value=re.sub(r"(?:translation|translated|hack|romhack|t\+[^ ]+|h\+[^ ]+)"," ",value,flags=re.I)
        return re.sub(r"[^a-z0-9]+"," ",value.casefold()).strip()

    @staticmethod
    def _make_scan_id(profile): return f"scan_{time.strftime('%Y%m%d_%H%M%S')}_{abs(hash((profile.profile_id,time.time_ns()))) % 100000:05d}"

    @staticmethod
    def _write_jsonl(stream,record): stream.write(json.dumps(record,ensure_ascii=False,separators=(",", ":"))+"\n")

    @classmethod
    def _emit(cls,evidence,result,stream):
        result.status_counts[evidence.status]+=1; result.evidence.append(evidence); cls._write_jsonl(stream,{"record_type":"evidence",**cls._serialize(evidence)})
    ...
    def _log_error(result,stream,path,exc):
        result.errors+=1; result.status_counts["ERROR"]+=1; NoIntroScanService._write_jsonl(stream,{"record_type":"error","path":str(path),"error":f"{type(exc).__name__}: {exc}"})
