...
class StableRomScanService(RomScanService):
    ...
    def _hash_stream(self, stream):
        ...
        while True:
            ...
            digest.update(chunk)
            self._heartbeat(f"SCAN | processando | machine={getattr(self, '_heartbeat_machine', '')} | ROM={getattr(self, '_heartbeat_rom', '') or '-'} | SHA1={total / 1024 / 1024:.1f} MiB")
    ...
    def _scan_mame_resumable(self, profile, db_path, resume=True):
        ...
        with path.open("a", encoding="utf-8", newline="\n") as stream, ThreadPoolExecutor(max_workers=workers, thread_name_prefix="mame-scan") as executor:
            futures = {executor.submit(self._scan_machine_with_heartbeat, m, db_path, int(import_id), sources, columns): m for m in pending}
            done = len(completed)
    ...
    def _completed_machines(path, machines, db_path):
        ...
        with sqlite3.connect(db_path) as c:
            expected = {str(n): int(k) for n, k in c.execute(f"SELECT m.name,COUNT(r.id) FROM mame_machine m LEFT JOIN mame_rom r ON r.machine_id=m.id WHERE m.name IN ({ph}) GROUP BY m.name", tuple(machines))}
        completed = set()
        current = None
        count = 0
        try:
