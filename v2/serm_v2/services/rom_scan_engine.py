...
class StableRomScanService(RomScanService):
    ...
    O scan continua sem filtros. JSONL é atualizado após cada machine concluída;
    um arquivo compatível incompleto pode ser reutilizado em uma nova execução.
    """
    ...
    def _hash_stream(self, stream):
        ...
        while True:
            ...
            digest.update(chunk)
            self._heartbeat(
                f"SCAN | processando | machine={self._heartbeat_machine} | "
                f"ROM={self._heartbeat_rom or '-'} | SHA1={total / 1024 / 1024:.1f} MiB"
            )
    ...
    def _scan_mame_resumable(self, profile, db_path):
        with sqlite3.connect(db_path) as connection:
            ...
            machine_names = [str(r[0]) for r in connection.execute(
        ...
        result = ScanResult(
            ...
            source=str(profile.source), system=str(profile.system), started_at=time.time(),
            scan_type=scan_type, catalog_hash=str(source_hash),
        ...
        self._log(
            "INFO",
            f"SCAN | MAME | retomada={bool(completed)} | "
            f"concluídas={len(completed):,}/{len(machine_names):,} | pendentes={len(pending):,}",
        )
        ...
        with stream_path.open("a", encoding="utf-8", newline="\n") as stream:
            with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="mame-scan") as executor:
                futures = {
                    executor.submit(
                        self._scan_machine, name, db_path, int(import_id),
                        sources, classification_columns
                    ): name
                    for name in pending
                }
                done = len(completed)
            ...
            if not self._cancelled:
                self._write_jsonl(stream, {
                    "record_type": "scan_end", "status": "completed",
                    "finished_at": time.time(), "status_counts": dict(result.status_counts),
    ...
    def _find_resume_stream(self, result, profile, sources, machine_names):
        ...
        for path in sorted(root.glob("scan_*.jsonl"), key=lambda p: p.stat().st_mtime, reverse=True):
            ...
            if header.get("record_type") != "header":
                continue
            if str(header.get("profile_id")) != str(profile.profile_id):
                continue
            if str(header.get("source")).casefold() != str(profile.source).casefold():
                continue
            if str(header.get("system")) != str(profile.system):
                continue
            if str(header.get("scan_type")) != str(result.scan_type):
                continue
            if str(header.get("catalog_hash")) != str(result.catalog_hash):
                continue
            if str(header.get("catalog_label")) != str(result.catalog_label):
                continue
            ...
            self._log(
                "INFO",
                f"RESUME | {path.name} | última machine="
                f"{self._last_completed(completed, machine_names) or 'nenhuma'}",
            )
        ...
        with path.open("w", encoding="utf-8", newline="\n") as stream:
            self._write_jsonl(stream, {
                "record_type": "header", "format": "SERM-SCAN-V2",
                ...
                "source_paths": wanted_paths, "machine_count_expected": len(machine_names),
                "metadata": {"validation": "expected_driven", "persist_mode": "streaming",
                             "filters_applied": False, "resumable": True},
            })
        return path, set()
