...
class StableRomScanService(RomScanService):
    ...
    def _hash_stream(self,stream):
        ...
        while True:
            ...
            if not chunk: break
            total+=len(chunk); crc=zlib.crc32(chunk,crc); digest.update(chunk)
    ...
    def scan(self,profile,*,catalog_items=(),database=None):
        ...
        return self._scan_mame_resumable(profile,database or database_path(),resume=True)
    def scan_new(self,profile,*,database=None):
        ...
        return self._scan_mame_resumable(profile,database or database_path(),resume=False)
    ...
    def _scan_mame_resumable(self,profile,db_path,resume=True):
        ...
        stream_path,completed=self._find_resume_stream(result,profile,sources,machine_names,db_path) if resume else self._create_new_stream(result,profile,sources,machine_names)
        result.evidence_stream_path=str(stream_path); pending=[m for m in machine_names if m not in completed]
        self._log("INFO",f"SCAN | MAME | modo={'retomada' if resume else 'novo'} | concluídas={len(completed):,}/{len(machine_names):,} | pendentes={len(pending):,}")
        ...
        with stream_path.open("a",encoding="utf-8",newline="\n") as stream:
            with ThreadPoolExecutor(max_workers=workers,thread_name_prefix="mame-scan") as executor:
                futures={executor.submit(self._scan_machine_with_heartbeat,name,db_path,int(import_id),sources,classification_columns):name for name in pending}
                done=len(completed)
                for future in as_completed(futures):
                    ...
                    if self.progress_callback: self.progress_callback(done,len(machine_names))
    ...
    def _create_new_stream(self,result,profile,sources,machine_names):
    ...
    def _find_resume_stream(self,result,profile,sources,machine_names,db_path):
        ...
        for path in sorted(root.glob("scan_*.jsonl"),key=lambda p:p.stat().st_mtime,reverse=True):
            try:
                with path.open("r",encoding="utf-8") as stream: header=json.loads(stream.readline())
            except (OSError,ValueError,UnicodeDecodeError): continue
            if header.get("record_type")!="header" or str(header.get("profile_id"))!=str(profile.profile_id) or str(header.get("source")).casefold()!=str(profile.source).casefold() or str(header.get("system"))!=str(profile.system): continue
            ...
            completed=self._completed_machines(path,machine_names,db_path); result.scan_id=str(header.get("scan_id") or result.scan_id); self._log("INFO",f"RESUME | {path.name} | última machine={self._last_completed(completed,machine_names) or 'nenhuma'}"); return path,completed
        return self._create_new_stream(result,profile,sources,machine_names)
    @staticmethod
    def _completed_machines(path,machine_names,db_path):
        valid=set(machine_names)
        if not valid:return set()
        ph=",".join("?" for _ in machine_names)
        with sqlite3.connect(db_path) as c: expected={str(n):int(k) for n,k in c.execute(f"SELECT m.name,COUNT(r.id) FROM mame_machine m LEFT JOIN mame_rom r ON r.machine_id=m.id WHERE m.name IN ({ph}) GROUP BY m.name",tuple(machine_names))}
