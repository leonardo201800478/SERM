...
class ScanFilterService:
    ...
    @classmethod
    def apply_mame(cls, scan_path: Path, profile, fundamental_values: dict[str, bool]) -> dict:
        ...
        enabled = {key for key, default in DEFAULT_FILTERS.items() if bool(fundamental_values.get(key, default))}
        kept: list[dict] = []
        reasons = Counter()
        for item in evidence:
            reason = cls._selection_reason(item, profile, enabled)
            if reason:
                reasons[reason] += 1
            else:
                kept.append(item)
        ...
        result = {
            ...
            "profile_id": str(profile.profile_id), "source": payload.get("source"), "system": payload.get("system"),
            "scan_type": scan_type, "catalog_label": payload.get("catalog_label"), "catalog_hash": payload.get("catalog_hash"),
            "source_scan_file": str(scan_path), "created_at": time.time(), "input_count": len(evidence),
            "output_count": len(kept), "filtered_count": len(evidence) - len(kept), "filter_counts": dict(reasons),
            "filters": {
    ...
    @classmethod
    def preview_mame(cls, scan_path: Path, profile, fundamental_values: dict[str, bool]) -> dict:
        payload = ScanFileRepository.load(scan_path)
        evidence = list(payload.get("evidence", []))
        enabled = {key for key, default in DEFAULT_FILTERS.items() if bool(fundamental_values.get(key, default))}
        reasons = Counter()
        ...
        for item in evidence:
            ...
            if reason:
                ...
                kept += 1
        return {"catalog_label": payload.get("catalog_label"), "scan_type": payload.get("scan_type"),
                "input_count": len(evidence), "output_count": kept, "filtered_count": len(evidence) - kept,
                "filter_counts": dict(reasons), "status_counts": dict(payload.get("status_counts") or {})}

    @classmethod
    def _selection_reason(cls, item: dict, profile, enabled: set[str]) -> str | None:
