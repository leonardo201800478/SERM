...
class RomScanService:
    ...
    def estimate_mame(self, profile, *, database: Path | None = None) -> dict[str, int | str | None]:
        ...
        if scan_type in {"software", "both"} and not has_software:
            return {"machines": int(machines), "roms": int(roms), "optional_roms": int(optional), "disks": int(disks), "catalog_roms": int(roms), "catalog_hash": str(source_hash), "scan_type": scan_type, "catalog_label": str(build or source_hash[:12]), "error": "O catálogo de Software Lists do MAME ainda não foi importado para o banco V2."}
    ...
    def _scan_mame(self, result: ScanResult, profile, db_path: Path, sources: list[Path], stream_path: Path) -> None:
        if not db_path.is_file():
        ...
        with sqlite3.connect(db_path) as connection:
            ...
            if result.scan_type == "both":
                self._log("WARNING", "SCAN | Ambos ainda usa somente a parte Arcade; Software Lists será adicionada quando normalizada.")
            machine_names = [str(row[0]) for row in connection.execute("SELECT name FROM mame_machine WHERE import_id=? ORDER BY name", (import_id,))]
            classification_columns = {str(row[1]) for row in connection.execute("PRAGMA table_info(mame_classification)")}
        ...
        with stream_path.open("w", encoding="utf-8", newline="\n") as stream:
            ...
            for offset in range(0, total, batch_size):
                ...
                with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="mame-scan") as executor:
                    futures = {executor.submit(self._scan_machine, name, db_path, int(import_id), sources, classification_columns): name for name in batch}
    ...
    def _scan_machine(self, machine: str, db_path: Path, import_id: int, sources: list[Path], classification_columns: set[str]) -> _MachineResult:
        unit = _MachineResult(machine)
        with sqlite3.connect(db_path) as connection:
            connection.row_factory = sqlite3.Row
            rows = connection.execute("""SELECT m.name AS machine_name, m.cloneof, r.id AS rom_id, r.name AS rom_name, r.size, lower(coalesce(r.crc,'')) AS expected_crc, lower(coalesce(r.sha1,'')) AS expected_sha1, lower(coalesce(r.md5,'')) AS expected_md5, r.merge, r.optional, m.isbios, m.isdevice, m.ismechanical, m.runnable, m.id AS machine_id FROM mame_machine m JOIN mame_rom r ON r.machine_id=m.id WHERE m.import_id=? AND m.name=? ORDER BY r.name""", (import_id, machine)).fetchall()
            machine_id = int(rows[0]["machine_id"]) if rows else None
            categories: tuple[str, ...] = ()
            if machine_id is not None and {"machine_id", "category"}.issubset(classification_columns):
    ...
    def _scan_zip(self, path: Path, expected_by_name: dict[str, list[_ExpectedRom]], unit: _MachineResult) -> None:
        try:
            with zipfile.ZipFile(path, "r") as archive:
                infos = {Path(info.filename).name.casefold(): info for info in archive.infolist() if not info.is_dir()}
    ...
    def _scan_loose(self, machine_dir: Path, expected_by_name: dict[str, list[_ExpectedRom]], unit: _MachineResult) -> None:
        for name, candidates in expected_by_name.items():
            if self._cancelled:
                return
            ...
            for item in candidates:
                try:
                    ...
                    status = "CURRENT" if (item.size <= 0 or size == item.size) and (not item.crc or crc == item.crc) else "WRONG"
                    actual_sha1 = ""
                    if status == "CURRENT" and item.sha1:
    ...
    def _write_machine(self, stream, unit: _MachineResult) -> None:
        self._write_jsonl(stream, {"record_type": "machine", "machine": unit.machine, "files_examined": unit.files_examined, "archives_examined": unit.archives_examined, "items_examined": unit.items_examined, "errors": unit.errors})
        for evidence in unit.records:
            self._write_jsonl(stream, {"record_type": "evidence", "machine_name": evidence.machine_name, "rom_name": evidence.rom_name, "status": evidence.status, "expected_size": evidence.expected_size, "actual_size": evidence.actual_size, "expected_crc": evidence.expected_crc, "actual_crc": evidence.actual_crc, "expected_sha1": evidence.expected_sha1, "actual_sha1": evidence.actual_sha1, "expected_md5": evidence.expected_md5, "actual_md5": evidence.actual_md5, "path": evidence.path, "archive_path": evidence.archive_path, "archive_member": evidence.archive_member, "merge_name": evidence.merge_name, "optional": evidence.optional, "message": evidence.message, "error": evidence.error, "categories": list(evidence.categories), "cloneof": evidence.cloneof, "isbios": evidence.isbios, "isdevice": evidence.isdevice, "ismechanical": evidence.ismechanical, "runnable": evidence.runnable})
    ...
    def _scan_generic_physical(self, result: ScanResult, profile, sources: list[Path], stream_path: Path, catalog_items: Iterable[ScanItem]) -> None:
        with stream_path.open("w", encoding="utf-8", newline="\n") as stream:
            ...
            for item in catalog_items:
                if self._cancelled:
                    break
                evidence = ScanEvidence(machine_name=item.machine_name, rom_name=item.rom_name, status="MISSING", expected_size=item.size, expected_crc=item.crc32.casefold(), expected_sha1=item.sha1.casefold(), optional=item.optional, message="ROM não encontrada")
                self._write_jsonl(stream, {"record_type": "evidence", "machine_name": evidence.machine_name, "rom_name": evidence.rom_name, "status": evidence.status, "expected_size": evidence.expected_size, "actual_size": None, "expected_crc": evidence.expected_crc, "actual_crc": "", "expected_sha1": evidence.expected_sha1, "actual_sha1": "", "expected_md5": "", "actual_md5": "", "path": None, "archive_path": None, "archive_member": None, "merge_name": None, "optional": evidence.optional, "message": evidence.message, "error": None, "categories": []})
                result.status_counts["MISSING"] += 1
            self._write_jsonl(stream, {"record_type": "scan_end", "status": "cancelled" if self._cancelled else "completed", "status_counts": dict(result.status_counts)})
