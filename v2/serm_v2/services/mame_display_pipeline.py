"""Pipeline de ingestão do ListXML do MAME, com transação atômica e progresso."""
...
class MameDisplayPipeline:
    """Captura ListXML, preserva o XML bruto e normaliza o catálogo."""
    ...
    def _log(self, message: str) -> None:
        """Envia uma mensagem para a GUI ou chamador."""
        self.logger(message)

    @staticmethod
    def _bind_value(value: Any, *, operation: str, parameter: int, context: str = "") -> Any:
        """Garante que o SQLite receba somente valores escalares suportados."""
    ...
    def _execute(self, db: sqlite3.Connection, sql: str, params: Sequence[Any], *, operation: str, context: str = "") -> sqlite3.Cursor:
        """Executa SQL parametrizado após validar os valores."""
        normalized = tuple(
            self._bind_value(value, operation=operation, parameter=index, context=context)
        ...
        try:
            ...
            raise MameDisplayPipelineError(
                f"Falha no SQLite | operação={operation} | parâmetros={len(normalized)} | "
                f"contexto={context or 'não informado'} | {exc}"
            ) from exc
    ...
    def run(
        ...
        *,
        ...
        profiles = self._resolve_profiles(import_result[0], fallback)
        return {
    ...
    def _import_xml(self, xml_text: str, executable: Path, *, force: bool) -> tuple[int, str | None, int, int, Path, str]:
        """Importa o catálogo atomicamente, sem perder o XML bruto."""
        try:
        ...
        with sqlite3.connect(self.db_path) as db:
            ...
            # IMPORTANTE: _insert_node retorna (id, quantidade). Nunca passar a tupla inteira como FK.
            ...
            for machine_index, machine in enumerate(machines):
                ...
                d, r, k, s = self._normalize_machine(
                    db, machine_id, machine, machine_path, context=context
                )
                totals[1] += r
                totals[2] += k
                totals[3] += s
                ...
                processed = machine_index + 1
                if processed % self.PROGRESS_INTERVAL == 0 or processed == len(machines):
                    pct = processed * 100 / len(machines)
                    self._log(
                        f"MAME | PROGRESS | {processed:,}/{len(machines):,} | {pct:6.2f}% | "
                        f"nós={totals[0]:,} | ROMs={totals[1]:,} | disks={totals[2]:,} | "
                        f"samples={totals[3]:,} | displays={totals[4]:,}"
                    )

            db.commit()
            self._log(
                f"MAME | INFO | persistência concluída | máquinas={len(machines):,} | "
                f"nós={totals[0]:,} | ROMs={totals[1]:,} | disks={totals[2]:,} | "
                f"samples={totals[3]:,} | displays={totals[4]:,}"
            )
    ...
    def _insert_machine(self, db: sqlite3.Connection, import_id: int, machine: ET.Element, ingested_at: str, *, context: str) -> int:
        ...
        cur = self._execute(
            ...
            (
                ...
                machine.findtext("year"), machine.findtext("manufacturer"), ingested_at,
            ),
    ...
    def _insert_node(self, db: sqlite3.Connection, import_id: int, parent_id: int | None, machine_id: int | None, element: ET.Element, path: str) -> tuple[int, int]:
        """Insere recursivamente a árvore XML e retorna (node_id, quantidade)."""
        cur = self._execute(
            db,
            ...
            "VALUES(?,?,?,?,?,?,?,?)",
            (
                import_id, parent_id, machine_id, element.tag, 0, path,
                (element.text or "").strip() or None,
                json.dumps(dict(element.attrib), ensure_ascii=False, sort_keys=True),
            ),
        ...
        for child in element:
            ...
            _, child_count = self._insert_node(
    ...
    def _normalize_machine(self, db: sqlite3.Connection, machine_id: int, machine: ET.Element, machine_path: str, *, context: str) -> tuple[int, int, int, int]:
        ...
        displays = roms = disks = samples = 0
        for display_index, display in enumerate(machine.findall("display")):
            a = display.attrib
            width = self._int(a.get("width"))
            height = self._int(a.get("height"))
            refresh_raw = a.get("refresh") or a.get("refresh_hz")
            refresh = self._float(refresh_raw)
            node_id = self._find_node_id(
                db, machine_id, f"{machine_path}/display[{display_index}]"
            )
            self._execute(
                db,
                "INSERT INTO mame_display "
                "(machine_id,tag,type,rotate,width,height,refresh_hz,refresh_raw,pixclock,htotal,"
                "hbend,hbstart,vtotal,vbend,vbstart,hsync,vsync,xaspect,yaspect,orientation_raw,source,confidence,xml_node_id) "
                "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    machine_id, a.get("tag"), a.get("type"), a.get("rotate"), width, height,
                    refresh, refresh_raw, a.get("pixclock"), a.get("htotal"), a.get("hbend"),
                    a.get("hbstart"), a.get("vtotal"), a.get("vbend"), a.get("vbstart"),
                    a.get("hsync"), a.get("vsync"), self._int(a.get("xaspect")),
                    self._int(a.get("yaspect")), a.get("rotate"), "listxml", "authoritative", node_id,
                ),
                operation="mame_display",
                context=f"{context}/display[{display_index}]",
            )
            displays += 1
        for rom in machine.findall("rom"):
            self._insert_rom(db, machine_id, rom, context=context)
            roms += 1
        for disk in machine.findall("disk"):
            self._insert_disk(db, machine_id, disk, context=context)
            disks += 1
        for sample in machine.findall("sample"):
            self._execute(
                ...
                operation="mame_sample",
                context=context,
            )
            samples += 1
        return displays, roms, disks, samples
    ...
    def _insert_rom(self, db: sqlite3.Connection, machine_id: int, element: ET.Element, *, context: str) -> None:
        ...
        self._execute(
            ...
            "(machine_id,name,bios,size,crc,sha1,md5,merge,region,offset,status,optional,dispose) "
            "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (machine_id, a.get("name"), a.get("bios"), a.get("size"), a.get("crc"), a.get("sha1"),
             a.get("md5"), a.get("merge"), a.get("region"), a.get("offset"), a.get("status"),
             a.get("optional"), a.get("dispose")),
            operation="mame_rom", context=context,
        )
    ...
    def _insert_disk(self, db: sqlite3.Connection, machine_id: int, element: ET.Element, *, context: str) -> None:
        ...
        self._execute(
            ...
            operation="mame_disk", context=context,
        )

    @staticmethod
    def _find_node_id(db: sqlite3.Connection, machine_id: int, path: str) -> int | None:
        """Obtém o nó XML correspondente a um display."""
        row = db.execute(
    ...
    def _int(value: str | None) -> int | None:
        """Converte um valor inteiro opcional."""
        try:
    ...
    def _float(value: str | None) -> float | None:
        """Converte um valor decimal opcional."""
        if value is None:
            return None
        try:
            return float(value)
    ...
    def _load_fallbacks(resolution_ini: str | Path | None, vsync_ini: str | Path | None) -> dict[str, dict[str, dict[str, object]]]:
        """Carrega os arquivos INI de fallback."""
        return {
    ...
    def _parse_fallback(path: str | Path | None) -> dict[str, dict[str, object]]:
        """Lê seções INI simples sem alterar o conteúdo original."""
        if path is None:
            return {}
    ...
    def _resolve_profiles(self, import_id: int, fallback: dict[str, dict[str, dict[str, object]]]) -> dict[str, object]:
        """Gera perfis de display derivados dos dados autoritativos do ListXML."""
        now = datetime.now(timezone.utc).isoformat()
        generated = 0
        with sqlite3.connect(self.db_path) as db:
            db.execute("PRAGMA foreign_keys=ON")
            rows = db.execute(
                "SELECT d.id,d.machine_id,d.width,d.height,d.refresh_hz,d.rotate,d.xaspect,d.yaspect "
                "FROM mame_display d JOIN mame_machine m ON m.id=d.machine_id WHERE m.import_id=?",
                (import_id,),
            ).fetchall()
            for display_id, machine_id, width, height, refresh, rotate, xasp, yasp in rows:
                self._execute(
                    ...
                    "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    (
                        machine_id, display_id, self.PARSER_VERSION, width, height, refresh, rotate, xasp, yasp,
                    ...
                    operation="mame_machine_display_profile",
                    context=f"display={display_id}",
                )
                generated += 1
            db.commit()
    ...
    def _ensure_schema(db: sqlite3.Connection) -> None:
        """Valida o conjunto mínimo de tabelas antes de iniciar uma ingestão."""
        required = {
            "mame_listxml_import", "mame_xml_node", "mame_machine", "mame_rom",
            "mame_disk", "mame_display", "mame_machine_display_profile",
        }
        existing = {
