...
class MameDisplayPipeline:
    """Captura ListXML, preserva o XML bruto e cria o catálogo normalizado."""
    ...
    def run(self, executable: str | Path, *, resolution_ini: str | Path | None = None,
        ...
        self._log(f"MAME | executável={executable_path}")
        self._log("MAME | executando -listxml...")
        xml_text = self._run_mame(executable_path, timeout)
        self._log(f"MAME | ListXML recebido | bytes={len(xml_text.encode('utf-8')):,}")
        ...
        return {"mame_executable": str(executable_path), "mame_build": import_result[1],
                "machine_count": import_result[2], "display_count": import_result[3],
                "xml_path": str(import_result[4]), "source_hash": import_result[5], **resolution}

    @staticmethod
    def _run_mame(executable: Path, timeout: float) -> str:
        """Executa MAME sem shell e retorna o ListXML."""
        try:
            result = subprocess.run([str(executable), "-listxml"], cwd=executable.parent,
    ...
    def _import_xml(self, xml_text: str, executable: Path, *, force: bool) -> tuple[int, str | None, int, int, Path, str]:
        """Importa em uma única transação, com operações SQL preparadas e progresso periódico."""
        try:
        ...
        self._log(f"MAME | XML validado | máquinas={len(machines):,} | hash={source_hash[:16]}")
        ...
        with sqlite3.connect(self.db_path) as db:
            ...
            emulator_row = db.execute("SELECT id FROM emulator_definition WHERE slug='mame'").fetchone()
            ...
            display_count = 0
            rom_count = disk_count = sample_count = node_count = 1
            self._log(f"MAME | import_id={import_id} | iniciando persistência")
            ...
            for machine_index, machine in enumerate(machines):
                ...
                if (machine_index + 1) % self.PROGRESS_INTERVAL == 0 or machine_index + 1 == len(machines):
                    ...
                    self._log(f"MAME | ingestão | {machine_index + 1:,}/{len(machines):,} | {pct:6.2f}% | nós={node_count:,} | ROMs={rom_count:,} | displays={display_count:,}")

            db.commit()
            self._log(f"MAME | persistência concluída | máquinas={len(machines):,} | nós={node_count:,} | ROMs={rom_count:,} | disks={disk_count:,} | samples={sample_count:,} | displays={display_count:,}")
    ...
    def _insert_node(self, db: sqlite3.Connection, import_id: int, parent_id: int | None, machine_id: int | None, element: ET.Element, path: str) -> tuple[int, int]:
        ...
        for child in element:
            ...
            _, child_count = self._insert_node(db, import_id, node_id, machine_id, child, child_path)
            count += child_count
        return node_id, count

    def _normalize_machine(self, db: sqlite3.Connection, machine_id: int, machine: ET.Element, machine_path: str) -> tuple[int, int, int, int]:
        """Normaliza display, ROM, disk e sample e retorna seus totais."""
        display_count = rom_count = disk_count = sample_count = 0
    ...
    def _float(value: str | None) -> float | None:
        """Converte float opcional aceitando formatos do MAME."""
    ...
    def _parse_fallback(path: str | Path | None) -> dict[str, dict[str, object]]:
        """Lê seções INI com chave=valor ou chave valor."""
    ...
    def _ensure_schema(self, db: sqlite3.Connection) -> None:
        """Valida o schema mínimo antes da ingestão."""
