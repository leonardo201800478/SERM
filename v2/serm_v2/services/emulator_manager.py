...
class EmulatorManager:
    ...
    def discover(self) -> dict[str, EmulatorStatus]:
        """Detecta executável, diretório e versão."""
    ...
    def install(self, key: str, destination: Path, *, nightly: bool = False, progress=None, log=None) -> DownloadResult:
        ...
        release = self._release(key, nightly=nightly); assets_value = release.get("assets")
        ...
        version = str(release.get("tag_name") or release.get("name") or "unknown"); self._log(log, f"RELEASE | {key} | versão={version} | asset={asset['name']}")
...
class RetroArchManager:
    ...
    STABLE_CORES_ROOT = f"{BUILD_ROOT}/stable/cores/windows/{WINDOWS_ARCH}/"
    ...
    def latest_stable_version(cls) -> str:
        """Retorna a versão Stable mais recente."""
    ...
    def discover_nightly_archive(cls) -> tuple[str, str]:
        """Localiza o pacote Nightly Windows x64 mais recente."""
        base = f"{cls.BUILD_ROOT}/nightly/windows/{cls.WINDOWS_ARCH}/"; html = cls._download_text(base); filenames: list[str] = []
        for href in re.findall(r'href=["\']([^"\']+)["\']', html, re.I):
    ...
    def _parse_core_index(text: str, channel: str) -> tuple[CoreInfo, ...]:
        ...
        for line in text.splitlines():
            ...
            core_name = re.sub(r"_libretro\.dll$", "", filename.removesuffix(".zip"), flags=re.I)
            result.append(CoreInfo(filename=filename, core_name=core_name, date=date, crc32=crc.lower().removeprefix("0x").zfill(8), channel=channel))
        return tuple(sorted(result, key=lambda item: item.core_name.casefold()))

    @classmethod
    def is_legacy_core(cls, core: CoreInfo) -> bool:
        """Identifica snapshots históricos que não entram no modo atual."""
    ...
    def is_game_or_engine_core(cls, core: CoreInfo) -> bool:
        """Identifica ports, jogos individuais e game engines."""
        normalized = re.sub(r"[^a-z0-9]", "", core.core_name.casefold())
    ...
    def filter_cores(cls, cores: tuple[CoreInfo, ...], *, current_only: bool = True, hide_games: bool = True) -> tuple[CoreInfo, ...]:
        """Aplica os filtros de legado e jogos/engines."""
    ...
    def list_filtered_cores(self, *, include_beta: bool = False, current_only: bool = True, hide_games: bool = True, stable_version: str | None = None) -> tuple[CoreInfo, ...]:
        """Retorna Stable ou Stable+Nightly, mesclando pelo nome do core."""
    ...
    def installed_cores(cores_dir: Path | None) -> tuple[Path, ...]:
        """Lista DLLs libretro instaladas."""
    ...
    def compare_installed_cores(self, cores: tuple[CoreInfo, ...], cores_dir: Path | None) -> list[tuple[Path, CoreInfo | None, str]]:
        """Compara CRC32 local com o catálogo oficial."""
    ...
    def install_core(self, filename: str, destination: Path, *, channel: str = "nightly", stable_version: str | None = None, progress=None, log=None) -> Path:
        ...
        with tempfile.TemporaryDirectory(prefix="serm-core-") as temp_name:
            ...
            remote = next((core for core in self.list_cores(channel, stable_version) if core.filename.casefold() == filename.casefold()), None)
            if remote is None: temp_dll.unlink(missing_ok=True); raise RuntimeError(f"Core não encontrado no índice oficial: {filename}")
            if remote.crc32 != actual_crc: temp_dll.unlink(missing_ok=True); raise RuntimeError(f"CRC32 inválido para {target.name}: recebido={actual_crc}, esperado={remote.crc32}")
    ...
    def detect_7zip(cls) -> Path | None:
        """Localiza 7-Zip no PATH ou instalações padrão."""
        candidates: list[Path] = []
    ...
    def _flatten_single_root(source: Path) -> Path:
        """Remove o nível único RetroArch-Win64 quando existente."""
    ...
    def _merge_preserving(source: Path, destination: Path, excluded: set[str]) -> None:
        """Mescla instalação preservando configurações, saves e states."""
    ...
    def install_retroarch(self, destination: Path, *, channel: str = "nightly", stable_version: str | None = None, progress=None, log=None) -> Path:
        ...
        destination = Path(destination).expanduser().resolve(); destination.mkdir(parents=True, exist_ok=True); buildroot, version = self.buildroot(channel, stable_version)
