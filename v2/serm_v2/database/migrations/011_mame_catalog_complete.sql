...
-- Catálogo relacional completo derivado do ListXML.
...
CREATE TABLE IF NOT EXISTS mame_machine_metadata (
    machine_id INTEGER PRIMARY KEY REFERENCES mame_machine(id) ON DELETE CASCADE,
    emulation_status TEXT,
    driver_status TEXT,
    savestate TEXT,
    requires_artwork TEXT,
    unofficial TEXT,
    nosoundhardware TEXT,
    incomplete TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_rom (
    ...
    name TEXT NOT NULL, bios TEXT, size INTEGER, crc TEXT, sha1 TEXT, md5 TEXT,
    merge TEXT, region TEXT, offset TEXT, status TEXT, optional TEXT, dispose TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_disk (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    name TEXT NOT NULL, md5 TEXT, sha1 TEXT, merge TEXT, region TEXT,
    index_value TEXT, writable TEXT, status TEXT, optional TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_biosset (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    name TEXT, description TEXT, default_flag TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_device_ref (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    name TEXT, tag TEXT, mandatory TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_chip (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    type TEXT, tag TEXT, name TEXT, clock TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_display (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    tag TEXT, type TEXT, rotate TEXT, width INTEGER, height INTEGER,
    refresh_hz REAL, refresh_raw TEXT, pixclock TEXT, htotal TEXT, hbend TEXT,
    hbstart TEXT, vtotal TEXT, vbend TEXT, vbstart TEXT, hsync TEXT, vsync TEXT,
    xaspect INTEGER, yaspect INTEGER, orientation_raw TEXT,
...
CREATE TABLE IF NOT EXISTS mame_input (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    players INTEGER, coins INTEGER, service INTEGER, tilt INTEGER
);
...
CREATE TABLE IF NOT EXISTS mame_control (
    ...
    type TEXT, player INTEGER, buttons INTEGER, minimum INTEGER, maximum INTEGER,
    sensitivity INTEGER, keydelta INTEGER, reverse TEXT, ways TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_dipswitch (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    name TEXT, tag TEXT, mask TEXT, value TEXT, default_value TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_port (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    tag TEXT, type TEXT, mask TEXT, defvalue TEXT, condition TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_adjuster (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    name TEXT, default_value TEXT, min_value TEXT, max_value TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_driver (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    status TEXT, emulation TEXT, color TEXT, sound TEXT, graphic TEXT,
    cocktail TEXT, protection TEXT, savestate TEXT, requires_artwork TEXT,
    unofficial TEXT, incomplete TEXT, notes TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_slot_option (
    ...
    name TEXT NOT NULL, devname TEXT, is_default TEXT
);
...
CREATE TABLE IF NOT EXISTS mame_softwarelist (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    machine_id INTEGER NOT NULL REFERENCES mame_machine(id) ON DELETE CASCADE,
    tag TEXT, name TEXT, status TEXT, filter TEXT
);
