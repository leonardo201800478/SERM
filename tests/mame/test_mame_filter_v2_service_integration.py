"""Testes do filtro MAME que dependem do catálogo relacional real.

Executados pela suíte estendida porque dependem do banco SERM já populado.
"""
...
def _state(**overrides):
    values = dict(
        ...
        year_from=None, year_to=None, type_filter="all", mamecab_only=True,
...
def test_mame_snapshot_is_converted_without_legacy_constructor_arguments():
    games = MameFilterV2Service._games({"evidence": [{"status": "CURRENT", "machine_name": "pacman", "description": "Pac-Man", "categories": ["Arcade"], "playability": "fully_playable", "working": "yes", "isbios": "no", "isdevice": "no"}]})
    assert len(games) == 1
    game = games[0]
...
def test_title_query_is_inclusive_and_machine_level():
    games = MameFilterV2Service._games({"evidence": [{"machine_name": "pacman", "description": "Pac-Man", "categories": ["Arcade"]}, {"machine_name": "galaga", "description": "Galaga", "categories": ["Arcade"]}]})
    result = ArcadeFilterEngine().apply(games, MameFilterV2Service._rules(_state(title_query="pacman"), games))
...
def test_non_matching_advanced_query_produces_zero_machines():
    games = MameFilterV2Service._games({"evidence": [{"machine_name": "pacman", "description": "Pac-Man", "categories": ["Arcade"]}]})
    result = ArcadeFilterEngine().apply(games, MameFilterV2Service._rules(_state(title_query="this-machine-does-not-exist"), games))
    assert result.included_count == 0


def test_mamecab_only_is_inclusive_arcade_filter_and_can_be_disabled():
    games = MameFilterV2Service._games({"evidence": [{"machine_name": "pacman", "description": "Pac-Man", "categories": ["Arcade"]}, {"machine_name": "nes", "description": "Nintendo Entertainment System", "categories": ["Console"]}]})
