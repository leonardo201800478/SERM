...
def main() -> int:
    ...
    try:
        print("\n[1/5] ESTRUTURA")
        exists = db.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='mame_resolution'"
        ).fetchone()
        ...
        print("\n[3/5] CARDINALIDADE")
        total = db.execute(
            "SELECT COUNT(*) FROM mame_resolution WHERE source_document_id=?", (source_id,)
        ).fetchone()[0]
        resolved = db.execute(
            "SELECT COUNT(*) FROM mame_resolution WHERE source_document_id=? AND resolved_status='resolved'",
            (source_id,),
        ).fetchone()[0]
        unresolved = db.execute(
            "SELECT COUNT(*) FROM mame_resolution WHERE source_document_id=? AND resolved_status='unresolved'",
            (source_id,),
        ).fetchone()[0]
        machines = db.execute("SELECT COUNT(*) FROM mame_machine").fetchone()[0]
        distinct = db.execute(
            "SELECT COUNT(DISTINCT machine_id) FROM mame_resolution WHERE source_document_id=? AND resolved_status='resolved'",
            (source_id,),
        ).fetchone()[0]
        ...
        print("\n[4/5] INTEGRIDADE")
        orphan = db.execute(
            """SELECT COUNT(*) FROM mame_resolution r
                              LEFT JOIN mame_machine m ON m.id=r.machine_id
                              WHERE r.source_document_id=? AND r.resolved_status='resolved' AND m.id IS NULL""",
            (source_id,),
        ).fetchone()[0]
        mismatch = db.execute(
            """SELECT COUNT(*) FROM mame_resolution r JOIN mame_machine m ON m.id=r.machine_id
                                WHERE r.source_document_id=? AND r.resolved_status='resolved' AND r.machine_name<>m.name""",
            (source_id,),
        ).fetchone()[0]
        invalid = db.execute(
            "SELECT COUNT(*) FROM mame_resolution WHERE source_document_id=? AND (width<=0 OR height<=0)",
            (source_id,),
        ).fetchone()[0]
        duplicate = db.execute(
            """SELECT COUNT(*) FROM (SELECT machine_name, COUNT(*) n FROM mame_resolution
                                  WHERE source_document_id=? GROUP BY machine_name HAVING n>1)""",
            (source_id,),
        ).fetchone()[0]
        ...
        print("\n[5/5] DISTRIBUIÇÃO")
        groups = db.execute(
            """SELECT width, height, COUNT(*) n FROM mame_resolution
                              WHERE source_document_id=? GROUP BY width, height ORDER BY n DESC LIMIT 10""",
            (source_id,),
        ).fetchall()
